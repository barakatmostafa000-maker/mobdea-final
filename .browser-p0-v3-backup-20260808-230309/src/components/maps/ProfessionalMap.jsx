  ...
  return (
    <div className="map-pro-stage" onClick={onStageClick} onDragOver={(event) => event.preventDefault()} onDrop={handleDrop}>
      <div ref={transformRef} className="map-pro-transform" style={{ transform: `scale(${zoom})` }}>
        <svg viewBox="0 0 1000 620" preserveAspectRatio="xMidYMid meet" className="map-pro-svg" role="img" aria-label={ariaLabel} style={{ background: '#0a3650' }}>
          <MapDefs />
          ...
          <g className="map-pro-coast-glow map-pro-country-layer">
              ...
              return (
                <g key={id}>
                  <path
                    ...
                    fill={active ? '#d9ad3f' : '#789a62'}
                    stroke={active ? '#fff1a5' : '#f0d478'}
                    strokeWidth={layerKey === 'borders' ? 2.2 : active ? 2.3 : 1.25}
                    opacity="1"
                    onClick={(event) => {
                      event.stopPropagation();
