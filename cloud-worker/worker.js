...
function corsHeaders(request, env) {
  ...
  const headers = {
    ...
    "Access-Control-Allow-Headers":
      "Content-Type,Authorization,X-Mobdea-Workspace,X-Mobdea-Client,X-Mobdea-Live-Token,X-Mobdea-Game-Token,X-Mobdea-Student-Token,If-Match,X-Mobdea-Asset-Name,X-Mobdea-Asset-Kind,X-Mobdea-Asset-Sha256,X-Mobdea-Asset-Size",
    "Access-Control-Expose-Headers":
      "ETag,X-Mobdea-Asset-Sha256,X-Mobdea-Asset-Size,X-Mobdea-Asset-Name,X-Mobdea-Asset-Kind",
...
async function encryptionKey(env) {
  ...
  if (secret.length < 32)
    throw new Error(
      "MOBDEA_ENCRYPTION_KEY must contain at least 32 characters.",
    );
...
function validateSyncPayload(payload, workspace) {
  ...
  if (
    ...
    Number(payload.schemaVersion) > 100
  )
    return false;
  if (
  ...
  if (payload.assetManifest !== undefined) {
    if (
      !Array.isArray(payload.assetManifest) ||
      payload.assetManifest.length > MAX_ASSET_COUNT
    )
      return false;
    for (const asset of payload.assetManifest) {
      if (!asset || !/^[a-zA-Z0-9._-]{1,100}$/.test(String(asset.id || "")))
        return false;
      if (!/^[a-f0-9]{64}$/.test(String(asset.sha256 || ""))) return false;
      if (
        !Number.isInteger(Number(asset.size)) ||
        Number(asset.size) <= 0 ||
        Number(asset.size) > MAX_ASSET_BYTES
      )
        return false;
    }
  }
...
async function rateLimit(
  ...
  const limit =
    overrideLimit || (write ? MAX_WRITES_PER_MINUTE : MAX_READS_PER_MINUTE);
...
async function verifyPbkdf2Secret(
  value,
  hashBase64,
  saltBase64,
  iterationsValue,
...
function safeStudentRecord(student = {}) {
  return {
    ...
    permissions:
      student.permissions && typeof student.permissions === "object"
        ? student.permissions
        : {},
...
function recordMatchesStudent(record = {}, student = {}) {
  ...
  const listIds = Array.isArray(record.studentIds)
    ? record.studentIds.map(String)
    : [];
  const listCodes = Array.isArray(record.studentCodes)
    ? record.studentCodes.map((value) => String(value).trim())
    : [];
  const hasExplicitScope =
    directIds.length > 0 ||
    directCodes.length > 0 ||
    Array.isArray(record.studentIds) ||
    Array.isArray(record.studentCodes);
  if (hasExplicitScope) {
    return (
      directIds.includes(studentId) ||
      listIds.includes(studentId) ||
      Boolean(
        studentCode &&
          (directCodes.includes(studentCode) ||
            listCodes.includes(studentCode)),
      )
    );
  }
  const gradeMatches =
    !record.grade ||
    !student.grade ||
    String(record.grade).trim() === String(student.grade).trim();
  const groupMatches =
    !record.group ||
    !student.group ||
    String(record.group).trim() === String(student.group).trim();
...
function recordExplicitlyMatchesStudent(record = {}, student = {}) {
  ...
  if (Array.isArray(record.studentIds))
    ids.push(...record.studentIds.map(String));
  ...
  const codes = [record.studentCode, record.selectedStudentCode]
    ...
    .map((value) => String(value).trim());
  if (Array.isArray(record.studentCodes))
    codes.push(...record.studentCodes.map((value) => String(value).trim()));
...
function collectStudentAssetIds(snapshot = {}) {
  ...
  for (const item of snapshot.contentLibrary || []) {
    add(item.assetId);
    add(item.examAssetId);
    add(item.thumbnailAssetId);
    add(item.recordingAssetId);
  }
  for (const item of snapshot.lessonRecordings || []) {
    add(item.boardAssetId);
    add(item.videoAssetId);
  }
  return ids;
}
...
function buildStudentSnapshot(stored = {}, student = {}) {
  ...
  const sameGrade = (value) =>
    !value ||
    !student.grade ||
    String(value).trim() === String(student.grade).trim();
  const own = (items = []) =>
    (Array.isArray(items) ? items : []).filter((item) =>
      recordExplicitlyMatchesStudent(item, student),
    );
  const visibleRecordings = (
    Array.isArray(data.lessonRecordings) ? data.lessonRecordings : []
  ).filter(
    (item) =>
      item.visibleToStudents !== false && recordMatchesStudent(item, student),
  );
  const contentLibrary = (
    Array.isArray(data.contentLibrary) ? data.contentLibrary : []
  ).filter((item) => sameGrade(item.grade));
  const sessions = (Array.isArray(data.sessions) ? data.sessions : [])
    .filter((item) => recordMatchesStudent(item, student))
    .map((item) => ({
      id: item.id,
      title: safeLiveText(item.title || "", 140),
      grade: safeLiveText(item.grade || "", 80),
      group: safeLiveText(item.group || "", 80),
      date: safeLiveText(item.date || "", 40),
      startTime: safeLiveText(item.startTime || "", 20),
      lessonId: item.lessonId ?? null,
    }));
  const customQuestionBank = (
    Array.isArray(data.customQuestionBank) ? data.customQuestionBank : []
  ).filter((item) => sameGrade(item.grade));
  return {
    ...
    settings: {
      ...
      cloudSync: {
        publicAppUrl: safeLiveText(
          data.settings?.cloudSync?.publicAppUrl || "",
          260,
        ),
      },
    },
...
async function authenticateStudentSession(request, env, workspace) {
  const token = String(
    request.headers.get("X-Mobdea-Student-Token") || "",
  ).trim();
...
async function handleStudentLogin(request, env, workspace) {
  if (!workspace)
    return json(request, env, { error: "workspace_required" }, 400);
  if (!(await rateLimit(request, env, workspace, true, 12, "student-login")))
  ...
  if (!Number.isSafeInteger(code) || code <= 0 || !/^\d{6,10}$/.test(pin))
    return json(request, env, { error: "invalid_credentials" }, 401);
  const stored = await readWorkspacePayload(env, workspace);
  if (!stored?.data)
    return json(request, env, { error: "workspace_not_ready" }, 404);
  const student = (stored.data.students || []).find(
    (item) => Number(item.code) === code,
  );
  if (!student) return json(request, env, { error: "student_not_found" }, 401);
  if (
    student.active === false ||
    !student.studentPinHash ||
    !student.studentPinSalt
  )
    return json(request, env, { error: "student_not_active" }, 403);
  const verified = await verifyPbkdf2Secret(
    pin,
    student.studentPinHash,
    student.studentPinSalt,
    student.studentPinIterations,
  );
  if (!verified)
    return json(request, env, { error: "invalid_credentials" }, 401);
  ...
  await env.MOBDEA_DATA.put(key, await encryptJson(session, env, key), {
    expirationTtl: STUDENT_SESSION_TTL_SECONDS,
  });
  return json(
    request,
    env,
    {
      ok: true,
      student: safeStudentRecord(student),
      studentToken: token,
      expiresAt,
      data: buildStudentSnapshot(stored, student),
    },
    200,
  );
}
...
async function handleStudentSnapshot(request, env, workspace) {
  ...
  if (!stored?.data)
    return json(request, env, { error: "workspace_not_ready" }, 404);
  const student = (stored.data.students || []).find(
    (item) => String(item.id) === String(session.studentId),
  );
...
async function handleStudentAsset(request, env, workspace, assetId) {
  const session = await authenticateStudentSession(request, env, workspace);
  if (!session) return json(request, env, { error: "unauthorized" }, 401);
  if (!env.MOBDEA_ASSETS || !/^[a-zA-Z0-9._-]{1,100}$/.test(assetId))
  ...
  if (!collectStudentAssetIds(snapshot).has(String(assetId)))
  ...
  const plaintext = await decryptBinary(
    ...
    metadata.iv,
    env,
    `asset:${workspace}:${assetId}`,
  );
  if (
    Number(metadata.size || 0) !== plaintext.byteLength ||
    (await sha256Hex(plaintext)) !== metadata.sha256
  )
    throw new Error("Stored asset integrity check failed.");
  const headers = assetResponseHeaders(request, env, metadata);
  headers["Content-Length"] = String(plaintext.byteLength);
  headers["Content-Disposition"] =
    `inline; filename*=UTF-8''${encodeURIComponent(metadata.name || "file")}`;
...
async function handleAssetStatus(request, env, auth) {
  if (!env.MOBDEA_ASSETS)
    return json(
      request,
      env,
      {
        error: "asset_storage_not_configured",
        message: "مخزن الملفات السحابي غير مفعّل.",
      },
      503,
    );
  if (!(await rateLimit(request, env, auth.workspace, false)))
    return json(request, env, { error: "rate_limited" }, 429, {
      "Retry-After": "60",
    });
  const body = await parseJsonBody(request, 80_000);
  const ids = Array.isArray(body?.ids)
    ? [...new Set(body.ids.map((id) => String(id || "")))]
    : [];
  if (
    !ids.length ||
    ids.length > MAX_ASSET_COUNT ||
    ids.some((id) => !/^[a-zA-Z0-9._-]{1,100}$/.test(id))
  ) {
  ...
  for (let offset = 0; offset < ids.length; offset += 25) {
    ...
    const records = await Promise.all(
      chunk.map(async (id) => ({
        id,
        object: await env.MOBDEA_ASSETS.head(
          `workspace/${auth.workspace}/${id}`,
        ),
      })),
    );
...
async function pruneWorkspaceAssets(
  env,
  workspace,
  manifest = [],
  previousManifest = [],
  ...
  const retained = new Set(
    manifest.map((asset) => String(asset.id || "")).filter(Boolean),
  );
  ...
  if (typeof env.MOBDEA_ASSETS.list === "function") {
    let cursor;
    do {
      const page = await env.MOBDEA_ASSETS.list({
        prefix,
        cursor,
        limit: 1000,
      });
      for (const object of page?.objects || [])
        if (!retained.has(String(object.key || "").slice(prefix.length)))
          obsolete.add(object.key);
      cursor = page?.truncated ? page.cursor : undefined;
    } while (cursor);
  }
  await Promise.allSettled(
    [...obsolete].map((key) => env.MOBDEA_ASSETS.delete(key)),
  );
}
...
async function handleAsset(request, env, auth, assetId) {
  ...
  if (!/^[a-zA-Z0-9._-]{1,100}$/.test(assetId))
    return json(request, env, { error: "not_found" }, 404);
  ...
  if (request.method === "HEAD") {
    ...
    if (!object)
      return new Response(null, {
        status: 404,
        headers: assetResponseHeaders(request, env),
      });
    return new Response(null, {
      status: 200,
      headers: assetResponseHeaders(request, env, object.customMetadata || {}),
    });
  }
  ...
  if (request.method === "PUT") {
    const declared = Number(
      request.headers.get("Content-Length") ||
        request.headers.get("X-Mobdea-Asset-Size") ||
        0,
    );
    if (declared <= 0 || declared > MAX_ASSET_BYTES)
    ...
    if (
      ...
      return json(request, env, { error: "invalid_asset_size" }, 400);
    const expectedHash = String(
      request.headers.get("X-Mobdea-Asset-Sha256") || "",
    ).toLowerCase();
    const actualHash = await sha256Hex(bytes);
    if (!/^[a-f0-9]{64}$/.test(expectedHash) || expectedHash !== actualHash)
      return json(request, env, { error: "asset_hash_mismatch" }, 400);
    const name =
      safeAssetMetadata(request.headers.get("X-Mobdea-Asset-Name")) || "file";
    const kind =
      safeAssetMetadata(request.headers.get("X-Mobdea-Asset-Kind"), 40) ||
      "resource";
    const type = String(
    ...
    await env.MOBDEA_ASSETS.put(key, encrypted.ciphertext, {
    ...
    return json(
      request,
      env,
      { ok: true, id: assetId, sha256: actualHash, size: bytes.byteLength },
      200,
    );
  }
...
async function handleSync(request, env, auth) {
  if (
    !(await rateLimit(request, env, auth.workspace, request.method === "PUT"))
  )
    return json(request, env, { error: "rate_limited" }, 429, {
      "Retry-After": "60",
    });
  ...
  if (request.method === "PUT") {
    ...
    if (!validateSyncPayload(payload, auth.workspace))
    ...
    const existing = existingEncrypted
      ? await decryptJson(existingEncrypted, env, aad)
      : null;
    const suppliedRevision = String(
      request.headers.get("If-Match") || payload.baseRevision || "",
    ).trim();
...
async function writeLiveRoom(env, room) {
  ...
  await env.MOBDEA_DATA.put(key, await encryptJson(room, env, key), {
...
async function writeLiveParticipant(env, room, participant) {
  ...
  await env.MOBDEA_DATA.put(key, await encryptJson(participant, env, key), {
    expirationTtl: liveExpirationTtl(room),
  });
}
...
async function authenticateLive(request, env, workspace, roomId) {
  ...
  const participant = await readLiveParticipant(
    env,
    workspace,
    roomId,
    participantId,
...
async function writeLiveEvent(env, room, event) {
  ...
  await env.MOBDEA_DATA.put(key, await encryptJson(record, env, key), {
    expirationTtl: liveExpirationTtl(room),
  });
  return record;
}
...
async function readGameParticipant(env, workspace, roomId, participantId) {
  const key = gameParticipantKey(workspace, roomId, participantId);
...
async function writeGameEvent(env, room, event = {}) {
  ...
  const record = {
    id: key.slice(gameEventPrefix(room.workspace, room.roomId).length),
    createdAtMs,
    createdAt: new Date(createdAtMs).toISOString(),
    ...event,
  };
...
async function authenticateGame(request, env, workspace, roomId) {
  const room = await readGameRoom(env, workspace, roomId);

  if (!room) return null;

  const token = String(request.headers.get("X-Mobdea-Game-Token") || "").trim();

  if (!token) return null;

  if ((await tokenHash(token)) === (await tokenHash(room.teacherToken))) {
    return {
      room,
      role: "teacher",
  ...
  const participantId = await env.MOBDEA_DATA.get(
    gameTokenIndexKey(workspace, roomId, await tokenHash(token)),
  );
...
async function createUniqueGameJoinCode(env, workspace) {
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    const existing = await env.MOBDEA_DATA.get(gameCodeKey(workspace, code));
...
async function handleCreateGameRoom(request, env, auth) {
  if (
    !(await rateLimit(request, env, auth.workspace, true, 20, "game-create"))
  ) {
    return json(request, env, { error: "rate_limited" }, 429, {
      "Retry-After": "60",
    });
  ...
  const ttlSeconds = Math.max(
    LIVE_ROOM_TTL_MIN,
    Math.min(LIVE_ROOM_TTL_MAX, Number(body.ttlSeconds || 21600)),
  );

  const roomId = randomToken().slice(0, 18);
  const teacherToken = randomToken();
  const joinCode = await createUniqueGameJoinCode(env, auth.workspace);
  ...
  const room = {
    ...
    title: safeLiveText(body.title || "تحدي مباشر", 140),
    mode: safeLiveText(body.mode || "individual", 30),
  ...
  await env.MOBDEA_DATA.put(gameCodeKey(auth.workspace, joinCode), roomId, {
...
async function handleJoinGameRoom(request, env, workspace) {
  if (!(await rateLimit(request, env, workspace, false, 60, "game-join"))) {
    return json(request, env, { error: "rate_limited" }, 429, {
      "Retry-After": "60",
    });
  }
  ...
  if (!joinCode) {
    return json(request, env, { error: "invalid_join_code" }, 400);
  }

  const roomId = await env.MOBDEA_DATA.get(gameCodeKey(workspace, joinCode));

  if (!roomId) {
    return json(request, env, { error: "game_room_not_found" }, 404);
  }
  ...
  if (room.status === "closed") {
    return json(request, env, { error: "game_room_closed" }, 410);
  }
  ...
  if (!displayName) {
    return json(request, env, { error: "display_name_required" }, 400);
  }
  ...
  const requestedTeam = safeLiveText(body.team || "", 40);

  const team =
    room.teamMode && room.teams.includes(requestedTeam) ? requestedTeam : "";
  ...
  await writeGameParticipant(env, room, participant);
...
async function listGameParticipants(env, workspace, roomId) {
  ...
  for (const item of page.keys || []) {
    ...
    const participant = await decryptJson(encrypted, env, item.name);
...
function buildGameLeaderboard(participants = []) {
  return participants.map((participant, index) => ({
    ...
    wrongAnswers: Number(participant.wrongAnswers || 0),
    answeredQuestions: Number(participant.answeredQuestions || 0),
...
function buildTeamLeaderboard(participants = []) {
  ...
  for (const participant of participants) {
    ...
    current.correctAnswers += Number(participant.correctAnswers || 0);
    current.members += 1;
...
async function handleGameParticipants(request, env, workspace, roomId) {
  const auth = await authenticateGame(request, env, workspace, roomId);

  if (!auth) {
    return json(request, env, { error: "unauthorized" }, 401);
  }

  const participants = await listGameParticipants(env, workspace, roomId);
...
async function handleGameRoomState(request, env, workspace, roomId) {
  const auth = await authenticateGame(request, env, workspace, roomId);
  ...
  if (request.method === "GET") {
    ...
    return json(request, env, {
      ok: true,
      room: publicGameRoom(auth.room),
      participant: auth.role === "participant" ? auth.participant : null,
      leaderboard: buildGameLeaderboard(participants),
      teamLeaderboard: buildTeamLeaderboard(participants),
    });
  }
  ...
  if (auth.role !== "teacher") {
    return json(request, env, { error: "teacher_required" }, 403);
  }
  ...
  if (body.pointsPerCorrect !== undefined) {
    room.pointsPerCorrect = Math.max(
      0,
      Math.min(1000, Number(body.pointsPerCorrect || 0)),
    );
  }
  ...
  if (room.status === "finished" || room.status === "closed") {
    room.finishedAt = room.finishedAt || new Date().toISOString();
...
function gameAnswerIsCorrect(answer, answerKey) {
  ...
  const acceptedAnswers = Array.isArray(answerKey) ? answerKey : [answerKey];

  return acceptedAnswers.some(
    (accepted) => normalizeGameAnswer(accepted) === submitted,
  );
}

async function handleGameAnswer(request, env, workspace, roomId) {
  const auth = await authenticateGame(request, env, workspace, roomId);
  ...
  if (auth.role !== "participant") {
    return json(request, env, { error: "participant_required" }, 403);
  }

  const room = auth.room;

  if (room.status !== "active") {
    return json(request, env, { error: "game_not_active" }, 409);
  }
  ...
  if (!questionId) {
    return json(request, env, { error: "question_not_ready" }, 409);
  }

  if (
    !room.answerKey ||
    (Array.isArray(room.answerKey) && room.answerKey.length === 0)
  ) {
    return json(request, env, { error: "answer_key_not_ready" }, 409);
  }
  ...
  if (!participant) {
    return json(request, env, { error: "participant_not_found" }, 404);
  }

  if (String(participant.lastQuestionId || "") === String(questionId)) {
    return json(
      request,
      env,
      {
  ...
  const answer = safeLiveText(body.answer ?? body.value ?? "", 300);

  if (!answer) {
    return json(request, env, { error: "answer_required" }, 400);
  }

  const correct = gameAnswerIsCorrect(answer, room.answerKey);

  const basePoints = Math.max(
    0,
    Math.min(1000, Number(room.pointsPerCorrect ?? 100)),
  );

  const elapsedMs = Math.max(0, Math.min(3600000, Number(body.elapsedMs || 0)));

  const timeLimitMs = Math.max(0, Number(room.state?.timeLimitMs || 0));

  let awardedPoints = correct ? basePoints : 0;

  if (correct && timeLimitMs > 0 && elapsedMs > 0 && elapsedMs < timeLimitMs) {
    const speedRatio = Math.max(0, (timeLimitMs - elapsedMs) / timeLimitMs);

    awardedPoints += Math.round(basePoints * 0.25 * speedRatio);
  }
  ...
  participant.correctAnswers =
    Number(participant.correctAnswers || 0) + (correct ? 1 : 0);

  participant.wrongAnswers =
    Number(participant.wrongAnswers || 0) + (correct ? 0 : 1);
...
async function handleGameEvents(request, env, workspace, roomId) {
  const auth = await authenticateGame(request, env, workspace, roomId);
  ...
  for (const item of page.keys || []) {
    ...
    const event = await decryptJson(encrypted, env, item.name);

    if (event && Number(event.createdAtMs || 0) > after) {
      events.push(event);
    }
  }
...
async function handleCloseGameRoom(request, env, workspace, roomId) {
  const auth = await authenticateGame(request, env, workspace, roomId);
...
async function handlePostGameEvent(request, env, workspace, roomId) {
  const auth = await authenticateGame(request, env, workspace, roomId);
  ...
  if (!type) {
    return json(request, env, { error: "event_type_required" }, 400);
  }
  ...
  if (auth.role !== "teacher" && !participantAllowed.has(type)) {
    return json(request, env, { error: "teacher_required" }, 403);
  }
  ...
  const event = await writeGameEvent(env, auth.room, {
    type,
    targetId: safeLiveText(body.targetId || "", 100),
    actorRole: auth.role,
    participantId: auth.role === "participant" ? auth.participantId : "teacher",
    data,
  });
...
async function handleCreateLiveRoom(request, env, auth) {
  if (
    !(await rateLimit(request, env, auth.workspace, true, 20, "live-create"))
  ) {
    return json(request, env, { error: "rate_limited" }, 429, {
      "Retry-After": "60",
    });
...
async function handleJoinLiveRoom(request, env, workspace, roomId) {
  ...
  if (!room || room.status !== "open") {
    return json(
      request,
      env,
      { error: "room_not_found", message: "الحصة غير متاحة أو انتهت." },
      404,
    );
  }
  ...
  if (!(await tokenEquals(String(body.joinCode || "").trim(), room.joinCode))) {
    return json(
      request,
      env,
      { error: "invalid_join_code", message: "كود دخول الحصة غير صحيح." },
      403,
    );
  }
  ...
  if (name.length < 2) {
    return json(
      request,
      env,
      { error: "invalid_name", message: "اكتب اسم الطالب قبل الدخول." },
      400,
    );
  }
  const participantId = randomToken().slice(0, 18);
...
function liveEventAllowedForParticipant(event, participantId) {
  ...
  if (event.targetId && !["all", participantId].includes(event.targetId))
    return false;
  if (event.sourceRole === "participant" && event.sourceId !== participantId) {
    return (
      ["reaction", "chat", "game-state"].includes(event.type) && !event.targetId
    );
  }
  return true;
}
...
async function handleLiveEvents(request, env, workspace, roomId) {
  ...
  if (
    !(await rateLimit(
      ...
      workspace,
      request.method === "POST",
      request.method === "POST" ? 180 : 600,
      `live-event-${request.method}`,
    ))
  ) {
    return json(request, env, { error: "rate_limited" }, 429, {
      "Retry-After": "10",
  ...
  if (request.method === "GET") {
    ...
    const page = await env.MOBDEA_DATA.list({
      prefix: liveEventPrefix(workspace, roomId),
      limit: 250,
    });
    const events = [];
    for (const key of page.keys || []) {
      ...
      if (
        ...
        !liveEventAllowedForParticipant(event, auth.participantId)
      )
        continue;
      events.push(event);
    }
    ...
    return json(request, env, {
      ...
      cursor: events.length
        ? Number(events[events.length - 1].createdAtMs || after)
        : after,
    });
  }
  ...
  if (request.method === "POST") {
    ...
    if (auth.role === "participant" && targetId && targetId !== "teacher")
      targetId = "teacher";
...
async function handleLiveParticipants(
  request,
  env,
  workspace,
  roomId,
  ...
  if (request.method === "GET" && !participantId) {
    const page = await env.MOBDEA_DATA.list({
      prefix: liveParticipantPrefix(workspace, roomId),
      limit: 250,
    });
    const participants = [];
    for (const key of page.keys || []) {
      ...
      const lastSeen = Date.parse(
        participant.lastSeenAt || participant.joinedAt || 0,
      );
      participants.push({
        ...participant,
        online:
  ...
  if (request.method === "PATCH" && participantId) {
    ...
    const allowedMicStates = new Set([
      "muted",
      "requested",
      "approved",
      "speaking",
    ...
    const next = {
      ...participant,
      micState: allowedMicStates.has(micState)
        ? micState
...
async function handleCreateShare(request, env, auth) {
  if (!(await rateLimit(request, env, auth.workspace, true)))
    return json(request, env, { error: "rate_limited" }, 429, {
      "Retry-After": "60",
    });
  const body = await parseJsonBody(request, MAX_SHARE_BYTES);
  if (!validateSharePayload(body))
    return json(
      request,
      env,
      { error: "invalid_payload", message: "محتوى المشاركة غير صالح." },
      400,
    );
  const ttl = Math.max(
    3600,
    Math.min(
      7 * 24 * 60 * 60,
      Number(body.expiresInSeconds || 7 * 24 * 60 * 60),
    ),
  );
  const token = randomToken();
  ...
  await env.MOBDEA_DATA.put(
    key,
    await encryptJson(record, env, `share:${auth.workspace}:${token}`),
    { expirationTtl: ttl },
  );
...
async function handleReadShare(request, env, token, workspace) {
  if (!(await rateLimit(request, env, "public", false)))
    return json(request, env, { error: "rate_limited" }, 429, {
      "Retry-After": "60",
    });
  if (!/^[a-zA-Z0-9_-]{20,80}$/.test(token) || !workspace)
    return json(request, env, { error: "not_found" }, 404);
  const key = `share:${workspace}:${token}`;
  const encrypted = await env.MOBDEA_DATA.get(key);
  if (!encrypted) return json(request, env, { error: "not_found" }, 404);
  const record = await decryptJson(
    encrypted,
    env,
    ...
    try {
      ...
      if (request.method === "GET" && shareMatch)
        return handleReadShare(
          ...
          env,
          shareMatch[1],
          validWorkspace(url.searchParams.get("workspace")),
        );

      const publicWorkspace = validWorkspace(
        request.headers.get("X-Mobdea-Workspace"),
      );
      if (url.pathname === "/student/login" && request.method === "POST")
        return handleStudentLogin(request, env, publicWorkspace);
      if (url.pathname === "/student/snapshot" && request.method === "GET")
        return handleStudentSnapshot(request, env, publicWorkspace);
      const studentAssetMatch = /^\/student\/assets\/([a-zA-Z0-9._-]+)$/.exec(
        url.pathname,
      );
      if (studentAssetMatch && request.method === "GET")
        return handleStudentAsset(
          request,
          env,
          publicWorkspace,
          studentAssetMatch[1],
      ...
      const liveWorkspace = validWorkspace(
        request.headers.get("X-Mobdea-Workspace"),
      );
      const liveJoinMatch = /^\/live\/rooms\/([a-zA-Z0-9_-]+)\/join$/.exec(
        url.pathname,
      );
      if (request.method === "POST" && liveJoinMatch) {
        return handleJoinLiveRoom(
          ...
          env,
          liveWorkspace,
          validLiveRoomId(liveJoinMatch[1]),
        );
      }
      const liveEventsMatch = /^\/live\/rooms\/([a-zA-Z0-9_-]+)\/events$/.exec(
        url.pathname,
      );
      if (liveEventsMatch && ["GET", "POST"].includes(request.method)) {
        return handleLiveEvents(
          request,
          env,
          liveWorkspace,
          validLiveRoomId(liveEventsMatch[1]),
        );
      }
      const liveParticipantMatch =
        /^\/live\/rooms\/([a-zA-Z0-9_-]+)\/participants(?:\/([a-zA-Z0-9_-]+))?$/.exec(
          url.pathname,
        );
      ...
      const liveRoomMatch = /^\/live\/rooms\/([a-zA-Z0-9_-]+)$/.exec(
        url.pathname,
      );
      if (liveRoomMatch && request.method === "DELETE") {
        return handleCloseLiveRoom(
          request,
          env,
          liveWorkspace,
          validLiveRoomId(liveRoomMatch[1]),
        );
      }
      ...
      if (url.pathname === "/game/rooms/join" && request.method === "POST") {
        return handleJoinGameRoom(request, env, gameWorkspace);
      }

      const gameStateMatch = /^\/game\/rooms\/([a-zA-Z0-9_-]+)\/state$/.exec(
        url.pathname,
      );

      if (gameStateMatch && ["GET", "PATCH"].includes(request.method)) {
      ...
      const gameParticipantsMatch =
        /^\/game\/rooms\/([a-zA-Z0-9_-]+)\/participants$/.exec(url.pathname);

      if (gameParticipantsMatch && request.method === "GET") {
      ...
      const gameAnswerMatch = /^\/game\/rooms\/([a-zA-Z0-9_-]+)\/answer$/.exec(
        url.pathname,
      );

      if (gameAnswerMatch && request.method === "POST") {
      ...
      const gameEventsMatch = /^\/game\/rooms\/([a-zA-Z0-9_-]+)\/events$/.exec(
        url.pathname,
      );

      if (gameEventsMatch && ["GET", "POST"].includes(request.method)) {
        const gameEventRoomId = validLiveRoomId(gameEventsMatch[1]);
        ...
        return handleGameEvents(request, env, gameWorkspace, gameEventRoomId);
      }

      const gameRoomMatch = /^\/game\/rooms\/([a-zA-Z0-9_-]+)$/.exec(
        url.pathname,
      );

      if (gameRoomMatch && request.method === "DELETE") {
      ...
      if (!auth) {
        ...
        return allowed
          ...
          : json(request, env, { error: "rate_limited" }, 429, {
              "Retry-After": "60",
            });
      }
      if (url.pathname === "/live/rooms" && request.method === "POST")
        return handleCreateLiveRoom(request, env, auth);

      /* MOBDEA_GAME_PRIVATE_ROUTES_V1 */
      if (url.pathname === "/game/rooms" && request.method === "POST") {
        return handleCreateGameRoom(request, env, auth);
      }

      if (url.pathname === "/assets/status" && request.method === "POST")
        return handleAssetStatus(request, env, auth);
      const assetMatch = /^\/assets\/([a-zA-Z0-9._-]+)$/.exec(url.pathname);
      if (
        assetMatch &&
        ["GET", "HEAD", "PUT", "DELETE"].includes(request.method)
      )
        return handleAsset(request, env, auth, assetMatch[1]);
