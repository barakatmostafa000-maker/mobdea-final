...
const toolOptions = [
  ...
  { key: 'geographical-term', label: 'مصطلح جغرافي', icon: Map },
  ...
  { key: 'place-term', label: 'مكان', icon: Map },
...
function BoardThemeDecor({ template }) {
  ...
  return (
    <svg className={`classmode-board-theme-art theme-${template}`} viewBox="0 0 1200 720" preserveAspectRatio="none" aria-hidden="true">
      {template === 'history' && <>
        <defs><linearGradient id="historyGold" x1="0" x2="1"><stop offset="0" stopColor="#7b4b1f"/><stop offset=".5" stopColor="#d5a34c"/><stop offset="1" stopColor="#7b4b1f"/></linearGradient></defs>
        <circle cx="1040" cy="128" r="58" fill="none" stroke="url(#historyGold)" strokeWidth="7" opacity=".28"/>
        <path d="M955 640 1080 385 1190 640ZM38 640 150 445 245 640Z" fill="#aa7739" opacity=".12" stroke="#7b4b1f" strokeWidth="5"/>
        <path d="M95 155v350m0-330h70m-55 0v330m855-390v390m0-370h75m-60 0v370" stroke="#7b4b1f" strokeWidth="12" opacity=".10"/>
        <path d="M28 112h1144M28 664h1144" stroke="url(#historyGold)" strokeWidth="5" opacity=".18"/>
        <g fill="none" stroke="#7b4b1f" strokeWidth="3" opacity=".13"><path d="M330 615c70-80 140-80 210 0s140 80 210 0 140-80 210 0"/><path d="M330 633c70-80 140-80 210 0s140 80 210 0 140-80 210 0"/></g>
      </>}
      {template === 'geography' && <>
  ...
  if (stamp.kind === 'text') {
    ...
    if (style !== 'plain') {
      ...
      if (style === 'historical') {
        ...
        ctx.beginPath(); ctx.arc(-boxWidth + 34, boxHeight / 2 - 4, 10, 0, Math.PI * 2); ctx.fill();
        ctx.beginPath(); ctx.moveTo(-boxWidth + 34, boxHeight / 2 + 17); ctx.lineTo(-boxWidth + 25, boxHeight / 2 + 1); ctx.lineTo(-boxWidth + 43, boxHeight / 2 + 1); ctx.closePath(); ctx.fill();
...
export default function ClassMode({ data, updateData, navigate }) {
  ...
  const selectedResourceType = useMemo(() => {
    ...
    if (selectedResource.type === 'textbook') return 'textbook';
    if (selectedResource.type === 'exams') return 'pdf';
  ...
  useEffect(() => {
    ...
    const mode = selectedResourceType === 'image'
      ? 'images'
      : selectedResourceType === 'video'
        ? 'videos'
        : selectedResourceType === 'audio'
          ? 'audio'
          : selectedResourceType === 'slides'
            ? 'slides'
            : ['document', 'file', 'link'].includes(selectedResourceType)
              ? 'files'
              : ['pdf', 'textbook'].includes(selectedResourceType)
                ? 'pdf'
                : null;
    if (['board', 'maps'].includes(contentMode)) return;
    if (mode && !modeResourceTypes[contentMode]?.includes(selectedResourceType)) {
      setContentMode(mode);
    }
  }, [selectedResource?.id, selectedResourceType]);
  ...
  const renderedPdf = usePdfPage(contentMode === 'pdf' && ['pdf', 'textbook'].includes(selectedResourceType) ? { blob: selectedResourceSource.blob, url: selectedResourceUrl, cacheKey: selectedResource?.assetId || selectedResource?.id || selectedResourceUrl } : null, classPage || 1);
  const boardLayerKey = contentMode === 'board'
    ? `board:${current?.id || 'session'}`
    : `${selectedResource?.id || 'blank'}:${classPage || 1}`;
  const displayResource = modeResourceTypes[contentMode]?.includes(selectedResourceType) ? { ...selectedResource, type: selectedResourceType } : null;
  ...
  }, [data.settings?.classResourceId, resources, selectedResourceId]);
  ...
  return (
    <ClassModeViewport className={`page classmode-scene classmode-final-layout ${fullscreen ? 'fullscreen' : ''}`} sceneRef={sceneRef}>
        ...
        <ClassModeViewport.Stage>
          ...
          <div className="classmode-board-frame">
            ...
            <div className={`classmode-board-surface ${boardToolsVisible ? 'with-tools' : ''}`}>
              ...
              <div className={`classmode-board-stage ${contentMode === 'board' ? 'has-lesson-ribbon' : ''}`}>
                ...
                {contentMode !== 'board' && contentMode !== 'maps' && (
                  <MediaNavigator
                    resources={resources}
          ...
          {boardToolsVisible && <div className="classmode-toolbar">
            ...
            <div className="classmode-tool-group compact classmode-text-style-row">
              ...
              <button className={textStyle === 'geography' ? 'active' : ''} onClick={() => activateStyledText('geography')} type="button"><Map size={15} /> مصطلح جغرافي</button>
              ...
              <button className={textStyle === 'place' ? 'active' : ''} onClick={() => activateStyledText('place')} type="button"><Map size={15} /> مكان</button>
