...
import {
  generateQuestionsForLessonBundle,
  removeGeneratedQuestions,
  upsertGeneratedQuestions,
} from "../services/contentQuestions";
import { deleteAsset, storeAsset } from "../services/assetStore";
import {
  autoDetectQuestionsFromPdfAsset,
  extractQuestionsFromPdfAsset,
} from "../services/pdfQuestionOcr";
...
function mergeOcrReviewQuestions(
  current = [],
  extracted = [],
  sourceKind,
  sourceAssetId,
  ...
  const byFingerprint = new Map(
    (Array.isArray(current) ? current : []).map((item) => [
      `${String(item.question || "")
        .trim()
        .toLowerCase()}|${item.page || ""}`,
      item,
    ]),
  );
  for (const [index, item] of (Array.isArray(extracted)
    ? extracted
    : []
  ).entries()) {
...
function LessonThumbnail({ lesson }) {
  ...
  return (
    <div className="library-lesson-thumbnail">
      {url ? (
        <img src={url} alt={lesson?.title || "صورة الدرس"} />
      ) : (
        <BookOpen size={28} />
      )}
    </div>
...
function LessonAccessAction({
  ...
  if (canManage)
    return (
      <button className="primary-btn" type="button" onClick={onOpenLessonMode}>
        <Sparkles size={15} /> فتح الحصة
      </button>
    );
  if (resources.length)
    return (
      <button className="primary-btn" type="button" onClick={onOpenViewer}>
        <PlayCircle size={15} /> عرض محتوى الدرس
      </button>
    );
  return (
    <span className="library-content-unavailable">لا يوجد ملف متاح للعرض</span>
  );
}
...
function StudentLessonResource({ resource, studentSession = null }) {
  ...
  if (!resource)
    return <div className="empty-state">لا يوجد محتوى في هذا الدرس.</div>;
  if (source.loading)
    return (
      <div className="empty-state">
        <File size={34} />
        <strong>{resource.title}</strong>
        <span>جارٍ تنزيل الملف من حساب الطالب…</span>
      </div>
    );
  if (!url)
    return (
      <div className="empty-state">
        ...
        <span>
          {source.error ||
            "الملف غير متاح على هذا الجهاز أو لم تتم مزامنته بعد."}
        </span>
      </div>
    );
  if (resource.type === "image")
    return (
      <img className="student-lesson-media" src={url} alt={resource.title} />
    );
  if (resource.type === "video")
    return (
      <video className="student-lesson-media" controls playsInline src={url} />
    );
  if (resource.type === "audio")
    return <audio className="student-lesson-audio" controls src={url} />;
  if (["pdf", "textbook"].includes(resource.type))
    return (
      <iframe
        className="student-lesson-document"
        title={resource.title}
        src={`${url}#toolbar=1&navpanes=0`}
      />
    );
  return (
    <div className="student-lesson-file-fallback">
      <MediaIcon type={resource.type} size={44} />
      <strong>{resource.title}</strong>
      <small>
        {resource.fileName ||
          resource.mimeType ||
          mediaLabels[resource.type] ||
          "ملف الدرس"}
      </small>
      <a
        ...
        rel="noopener noreferrer"
      >
        <ExternalLink size={15} /> فتح الملف
      </a>
    </div>
  );
}

function StudentLessonViewer({
  data,
  lesson,
  index,
  onIndex,
  ...
  const resources = getLessonModeResources(data, lesson?.grade, lesson?.id);
  const safeIndex = resources.length
    ? Math.max(0, Math.min(index, resources.length - 1))
    : 0;
  const resource = resources[safeIndex] || null;
  return (
    <div
      className="student-lesson-viewer-backdrop"
      role="presentation"
      onClick={onClose}
    >
      <section
        className="student-lesson-viewer"
        role="dialog"
        aria-modal="true"
        aria-label={`محتوى ${lesson?.title || "الدرس"}`}
        onClick={(event) => event.stopPropagation()}
      >
        <header>
          <div>
            <span className="eyebrow">محتوى الدرس</span>
            <h2>{lesson?.title}</h2>
            <small>
              {resource?.title || "لا يوجد ملف"}{" "}
              {resources.length
                ? `— ${safeIndex + 1} من ${resources.length}`
                : ""}
            </small>
          </div>
          <button
        ...
        <div className="student-lesson-viewer-stage">
        ...
        <footer>
          <button
            ...
            onClick={() =>
              onIndex((safeIndex - 1 + resources.length) % resources.length)
            }
          >
            <ChevronRight size={17} /> السابق
          </button>
          <div className="student-lesson-resource-tabs">
            {resources.map((item, itemIndex) => (
              <button
                ...
                title={item.title}
              >
                <MediaIcon type={item.type} size={16} />
          ...
          <button
            ...
            type="button"
            disabled={resources.length < 2}
            onClick={() => onIndex((safeIndex + 1) % resources.length)}
          >
...
function PermanentCard({
  ...
  return (
    <article
      className={`library-permanent-card ${hasResourceSource(resource || {}) ? "ready" : "missing"}`}
    >
      <div className="library-permanent-icon">
        <Icon size={30} />
      </div>
      <div className="library-permanent-copy">
        ...
        {resource ? (
          ...
          <small>لم يُرفع ملف لهذا الصف بعد.</small>
        )}
      </div>
      <div className="library-permanent-actions">
        ...
        {canManage && resource && (
          <button
            ...
            disabled={busy}
            onClick={onRemove}
            title="إزالة الملف مع إبقاء البطاقة"
          >
            <Trash2 size={16} />
          </button>
        )}
...
export default function ContentLibrary({ data, updateData, auth, navigate }) {
  ...
  const studentRecord = useMemo(() => {
    ...
    return (
      (data.students || []).find(
        (item) => Number(item.id) === Number(auth.studentId),
      ) ||
      (data.students || []).find(
        (item) => String(item.code) === String(auth.studentCode || ""),
      ) ||
      null
    );
  ...
  const [selectedGrade, setSelectedGrade] = useState(
    studentRecord?.grade ||
      ...
      grades.find((item) => item === defaultGrade) ||
      grades[0] ||
      defaultGrade,
  );
  const [expandedGrades, setExpandedGrades] = useState(
    () => new Set([selectedGrade]),
  );
  ...
  useEffect(() => {
    if (
      auth?.role === "student" &&
      studentRecord?.grade &&
      selectedGrade !== studentRecord.grade
    ) {
  ...
  const selectedTextbook = useMemo(
    () => getGradeTextbook(data, selectedGrade),
    [data, selectedGrade],
  );
  const selectedExams = useMemo(
    () => getGradeExams(data, selectedGrade),
    [data, selectedGrade],
  );
  const editingMedia = useMemo(
    () =>
      form.id
        ? getLessonMedia(data, form.id).filter(
            (item) => !removedMediaIds.includes(String(item.id)),
          )
        : [],
    [data, form.id, removedMediaIds],
  );

  const cleanupPending = async () => {
    await Promise.all(
      pendingMedia.map((item) => deleteAsset(item.assetId).catch(() => {})),
    );
    const current = (data.contentLibrary || []).find(
      (item) => String(item.id) === String(form.id),
    );
    const keep = new Set(
      [current?.thumbnailAssetId, current?.recordingAssetId]
        .filter(Boolean)
        .map(String),
    );
    const stagedSpecial = [form.thumbnailAssetId, form.recordingAssetId].filter(
      (id) => id && !keep.has(String(id)),
    );
    await Promise.all(
      [...replacedAssetIds, ...stagedSpecial]
        .filter(
          (id, index, list) =>
            id && !keep.has(String(id)) && list.indexOf(id) === index,
        )
  ...
  const openCreateLesson = (grade = selectedGrade) => {
    ...
    setOcrStatus("");
    setNotice(
      getGradeTextbook(data, grade)
        ? "سيستخدم الدرس كتاب الصف الرئيسي تلقائيًا."
        : "تنبيه: ارفع كتاب الصف الرئيسي قبل فتح صفحات الدرس داخل وضع الحصة.",
    );
  };
  ...
  const openEditLesson = (lesson) => {
    ...
    setForm({
      ...
      sequence:
        Array.isArray(lesson.sequence) && lesson.sequence.length
          ? lesson.sequence
          : [...defaultSequence],
    ...
    setOcrStatus("");
    setNotice(
      "يمكن تعديل بيانات الدرس وإضافة وسائط جديدة دون إعادة رفع كتاب المنهج.",
    );
  };
  ...
  const replaceFormAsset = async (file, type) => {
    ...
    try {
      ...
      if (isThumbnail && !String(file.type || "").startsWith("image/"))
        throw new Error("الصورة المصغرة يجب أن تكون ملف صورة.");
      if (
        type === "recording" &&
        !String(file.type || "").startsWith("audio/") &&
        !String(file.type || "").startsWith("video/")
      )
        throw new Error("تسجيل الدرس يجب أن يكون صوتًا أو فيديو.");
      ...
      if (form[idField])
        setReplacedAssetIds((current) => [...current, form[idField]]);
      setForm((current) => ({
        ...current,
        [idField]: asset.id,
      ...
      setNotice(
        `تم تجهيز ${isThumbnail ? "الصورة المصغرة" : "تسجيل الدرس"} للحفظ.`,
      );
    } catch (error) {
  ...
  const stageLessonMedia = async (files) => {
    ...
    try {
      ...
      await Promise.all(
        staged.map((item) => deleteAsset(item.assetId).catch(() => {})),
      );
  ...
  const autoDetectLessonOcr = async () => {
    ...
    const sourceLabel =
      sourceKind === "exams" ? "ملف الامتحانات الأساسي" : "كتاب الشرح الأساسي";
    if (!source?.assetId) {
      setNotice(`ارفع ${sourceLabel} لهذا الصف قبل البحث عن صفحات الأسئلة.`);
      return;
    }
    const lessonStartPage = Math.max(1, Number(form.pageStart || 1));
    const lessonEndPage = Math.max(
      lessonStartPage,
      Number(form.pageEnd || lessonStartPage),
    );
    setOcrRunning(true);
    const controller = new AbortController();
    ocrAbortRef.current = controller;
    setOcrStatus(
      `جارٍ البحث تلقائيًا عن صفحات الأسئلة قرب نهاية الدرس في ${sourceLabel}...`,
    );
    try {
      const result = await autoDetectQuestionsFromPdfAsset({
        ...
        onProgress: ({ stage, page }) => {
          if (stage === "staging-file") {
            setOcrStatus(
              "جارٍ نقل ملف PDF إلى محرك Android على دفعات آمنة للذاكرة...",
            );
          } else if (stage === "downloading-model") {
            setOcrStatus(
              "جارٍ تنزيل نموذج OCR العربي لأول مرة؛ سيُحفظ على الجهاز للاستخدام التالي.",
            );
          } else if (stage === "rendering") {
            setOcrStatus(
              `جارٍ تجهيز الصفحة ${page} للبحث عن التدريبات والأسئلة...`,
            );
          } else {
      ...
      if (!result.detected || !result.questionCount) {
        setOcrStatus(
          `لم يتم اكتشاف صفحات أسئلة مؤكدة تلقائيًا بين صفحات الدرس ${lessonStartPage}–${lessonEndPage}. يمكنك تحديد النطاق يدويًا ثم الضغط على استخراج.`,
        );
        return;
      }
...
${extracted}`
        ...
        ocrReviewQuestions: mergeOcrReviewQuestions(
          current.ocrReviewQuestions,
          contextualizeOcrQuestions(result.questions, {
            sourceKind,
      ...
      const pagesLabel =
        Array.isArray(result.detectedPages) && result.detectedPages.length
          ? result.detectedPages.join("، ")
          : `${result.startPage}–${result.endPage}`;
      setOcrStatus(
        `تم اكتشاف صفحات الأسئلة تلقائيًا: ${pagesLabel}. استُخرج ${result.questionCount} سؤالًا؛ منها ${result.answeredCount} بإجابة قابلة للاستخدام في الألعاب.`,
      );
      setNotice(
        "تم تحديد صفحات الأسئلة تلقائيًا. راجع النص المستخرج فقط قبل حفظ الدرس.",
      );
    } catch (error) {
  ...
  const runLessonOcr = async () => {
    ...
    const startPage = Math.max(
      1,
      Number(form.questionPageStart || form.pageStart || 1),
    );
    const endPage = Math.max(
      startPage,
      Number(form.questionPageEnd || form.pageEnd || startPage),
    );
    setOcrRunning(true);
    const controller = new AbortController();
    ocrAbortRef.current = controller;
    ...
    try {
      const result = await extractQuestionsFromPdfAsset({
        ...
        onProgress: ({ stage, page, totalPages }) => {
          if (stage === "staging-file") {
            ...
            setOcrStatus(
              `جارٍ قراءة الصفحة ${page} (${Math.max(1, totalPages)} صفحة في النطاق)...`,
            );
          }
        },
      });
      if (!result.questionCount) {
        setOcrStatus(
          "لم يعثر OCR على أسئلة واضحة في الصفحات المحددة. جرّب تضييق النطاق أو راجع جودة الصفحات.",
        );
        return;
      }
      const existing = String(form.questionText || "").trim();
      ...
      setOcrStatus(
        `تم استخراج ${result.questionCount} سؤالًا من ${sourceLabel}، الصفحات ${startPage}–${endPage}.${reviewMessage}`,
      );
      setNotice(
        "راجع النص المستخرج وصحح أي خطأ OCR؛ لن تدخل الأسئلة التي بلا إجابة صحيحة في الألعاب الآلية.",
      );
    } catch (error) {
    ...
    try {
      ...
      const old =
        kind === LIBRARY_KINDS.GRADE_TEXTBOOK
          ? getGradeTextbook(data, targetGrade)
          : getGradeExams(data, targetGrade);
      ...
      const contentLibrary = old
        ? (data.contentLibrary || []).map((item) =>
            String(item.id) === String(old.id) ? resource : item,
          )
      ...
      if (old?.assetId && old.assetId !== created.id)
        await deleteAsset(old.assetId).catch(() => {});
      const affectedLessons = getLessonsForGrade(snapshot, targetGrade).length;
      setNotice(
        `تم حفظ ${resource.title} بصورة دائمة وربطه بـ ${affectedLessons} درس، وتحديث أسئلة الألعاب وبنك الأسئلة.`,
      );
    } catch (error) {
  ...
  const removePermanentFile = async (kind, grade = selectedGrade) => {
    const resource =
      kind === LIBRARY_KINDS.GRADE_TEXTBOOK
        ? getGradeTextbook(data, grade)
        : getGradeExams(data, grade);
    if (!resource) return;
    const contentLibrary = (data.contentLibrary || []).map((item) =>
      String(item.id) === String(resource.id)
        ? {
            ...item,
            assetId: "",
            url: "",
    ...
    setNotice(
      "تمت إزالة الملف وتحديث أسئلة الدروس المرتبطة، وستظل البطاقة الدائمة متاحة لرفع بديل.",
    );
  };
  ...
  const saveLesson = async () => {
    ...
    const existing = (data.contentLibrary || []).find(
    ...
    const lesson = {
      ...
      questionPageStart: Math.max(
        pageStart,
        Number(form.questionPageStart || pageStart),
      ),
      questionPageEnd: Math.max(
        Number(form.questionPageStart || pageStart),
        Number(form.questionPageEnd || pageEnd),
      ),
      ...
      ocrReviewQuestions: Array.isArray(form.ocrReviewQuestions)
        ? form.ocrReviewQuestions
        : [],
    ...
    const keptItems = (data.contentLibrary || []).filter(
      (item) => !removedSet.has(String(item.id)),
    );
    const withLesson = existing
      ? keptItems.map((item) =>
          String(item.id) === String(lessonId) ? lesson : item,
        )
      : [...keptItems, lesson];
    ...
    try {
      ...
      const classResources = getLessonModeResources(
        snapshot,
        lesson.grade,
        lesson.id,
      );
      ...
      const removedAssets = (data.contentLibrary || [])
        ...
        .filter(Boolean);
      const oldAssets = replacedAssetIds.filter(
        (id) =>
          id &&
          id !== lesson.thumbnailAssetId &&
          id !== lesson.recordingAssetId,
      );
      await Promise.all(
        [...removedAssets, ...oldAssets].map((id) =>
      ...
      setEditorOpen(false);
      setNotice(
        "تم حفظ الدرس وظهر فورًا في وضع الحصة مع كتاب الشرح وملف الامتحانات والوسائط، وتم تحديث بنك الأسئلة والألعاب.",
      );
    } catch (error) {
  ...
  const deleteLesson = async (lesson) => {
    ...
    const ids = new Set([
      String(lesson.id),
      ...media.map((item) => String(item.id)),
    ]);
    const assets = [
      lesson.thumbnailAssetId,
      lesson.recordingAssetId,
      ...media.map((item) => item.assetId),
    ].filter(Boolean);
    const contentLibrary = (data.contentLibrary || []).filter(
      (item) => !ids.has(String(item.id)),
    );
    let customQuestionBank = removeGeneratedQuestions(
      data.customQuestionBank || [],
      lesson.id,
    );
    for (const item of media)
      customQuestionBank = removeGeneratedQuestions(
        customQuestionBank,
        item.id,
      );
    const settings =
      String(data.settings?.classLessonId) === String(lesson.id)
        ? {
            ...data.settings,
            classLessonId: "",
            classResourceId: "",
  ...
  const openInLessonMode = async (lesson) => {
    ...
    await updateData({
      ...
      settings: {
        ...
        classResourceQueue: resources.map((item) => ({
          id: item.id,
          title: item.title,
          type: item.type,
          lessonId: lesson.id,
    ...
    setNotice(
      resources.length
        ? "تم تجهيز الدرس وكتابه ووسائطه في وضع الحصة."
        : "تم تجهيز الدرس؛ ارفع كتاب الصف أو وسائطه لإظهار المحتوى.",
    );
    navigate?.("classMode");
  };
  ...
  const toggleSequence = (key) =>
    setForm((current) => ({
      ...current,
      sequence: current.sequence.includes(key)
        ? current.sequence.filter((item) => item !== key)
        : [...current.sequence, key],
    }));

  const filteredLessons = (grade) => {
    const query = search.trim().toLowerCase();
    return getLessonsForGrade(data, grade).filter(
      (lesson) =>
        !query ||
        [lesson.title, lesson.unit, lesson.notes, ...normalizeTags(lesson.tags)]
          .join(" ")
          .toLowerCase()
          .includes(query),
    );
  ...
  return (
    <section className="page content-page library-system-page library-v103">
      <input
        ...
        hidden
        onChange={(event) =>
          void uploadPermanent(
            LIBRARY_KINDS.GRADE_TEXTBOOK,
      ...
      <input
        ...
        multiple
        hidden
        accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.ppt,.pptx,.pps,.ppsx,.odp,.xls,.xlsx,.txt"
        onChange={(event) => void stageLessonMedia(event.target.files)}
      />
      <input
        ...
        hidden
        onChange={(event) =>
          void replaceFormAsset(event.target.files?.[0], "thumbnail")
        }
      />
      <input
        ...
        hidden
        onChange={(event) =>
          void replaceFormAsset(event.target.files?.[0], "recording")
        }
      />

      <div className="page-heading library-heading">
        <div>
          <span className="eyebrow">المصدر الموحد للمحتوى</span>
          <h2>المكتبة وإدارة الدروس</h2>
          <p>
            الكتاب والامتحانات والدروس ووسائطها تُدار من هنا، وتفتح تلقائيًا
            داخل وضع الحصة.
          </p>
        </div>
        <div className="library-heading-actions">
          <label>
            ...
            <select
              ...
              onChange={(event) => {
                ...
                setExpandedGrades(
                  (current) => new Set([...current, event.target.value]),
                );
              }}
            >
          ...
          {canManage && (
            <button
              className="primary-btn"
              type="button"
              onClick={() => openCreateLesson(selectedGrade)}
            >
              <Plus size={17} /> درس جديد
            </button>
          )}
        </div>
      </div>
      ...
      <div className="library-summary-strip">
        <article>
          <BookOpen />
          <span>كتب جاهزة</span>
          <strong>{summary.textbooks}</strong>
        </article>
        <article>
          <BookMarked />
          <span>ملفات امتحانات</span>
      ...
      <div className="library-permanent-grid">
        <PermanentCard
          kind={LIBRARY_KINDS.GRADE_TEXTBOOK}
          resource={selectedTextbook}
          grade={selectedGrade}
          canManage={canManage}
          busy={busy}
          onUpload={() => {
            permanentGradeRef.current = selectedGrade;
            textbookInputRef.current?.click();
          }}
          onRemove={() =>
            void removePermanentFile(
              LIBRARY_KINDS.GRADE_TEXTBOOK,
              selectedGrade,
            )
          }
        ...
        <PermanentCard
          ...
          onRemove={() =>
            void removePermanentFile(LIBRARY_KINDS.GRADE_EXAMS, selectedGrade)
          }
        />
      </div>
      ...
      <div className="library-search-bar">
        ...
        <input
          ...
          placeholder="ابحث باسم الدرس أو الوحدة أو الملاحظات"
        />
        <span>{summary.lessons} درس</span>
      </div>
      ...
      <div className="library-grade-accordions">
        {visibleGrades.map((grade) => {
          ...
          return (
            <section
              className={`library-grade-section ${opened ? "open" : ""}`}
              key={grade}
            >
              <div className="library-grade-header">
                <button
                  className="library-grade-toggle"
                  type="button"
                ...
                >
                  <div className="library-grade-title">
                    <span>{opened ? <ChevronUp /> : <ChevronDown />}</span>
                    <div>
                      <strong>{grade}</strong>
                      <small>
                        {lessons.length} درس • الكتاب{" "}
                        {hasResourceSource(textbook || {})
                          ? "جاهز"
                          : "غير مرفوع"}{" "}
                        • الامتحانات{" "}
                        {hasResourceSource(exams || {})
                          ? "جاهزة"
                          : "غير مرفوعة"}
                      </small>
                    </div>
                ...
                {canManage && (
                  <button
                    className="library-grade-add"
                    type="button"
                    onClick={() => openCreateLesson(grade)}
                  >
                ...
                <>
                  <div className="library-grade-permanent-row">
                    <PermanentCard
                      ...
                      busy={busy}
                      onUpload={() => {
                        permanentGradeRef.current = grade;
                        setSelectedGrade(grade);
                        textbookInputRef.current?.click();
                      }}
                      onRemove={() =>
                  ...
                  <div className="library-lessons-grid">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="library-lesson-main">
                            ...
                            <h3>{lesson.title}</h3>
                            <p>
                              {lesson.notes || "لا توجد ملاحظات للدرس."}
                              {lesson.homework
                            ...
                            <div className="library-lesson-meta">
                              <span>
                                <FileText size={14} /> ص {lesson.pageStart || 1}
                                –{lesson.pageEnd || lesson.pageStart || 1}
                              </span>
                              <span>
                                <FolderOpen size={14} /> {media.length} وسائط
                              </span>
                              <span>
                                <MapIcon size={14} />{" "}
                                {lesson.mapState
                            ...
                            <div className="library-media-chips">
                              {media.slice(0, 6).map((item) => (
                                <span key={item.id}>
                                  <MediaIcon type={item.type} size={13} />
                                  {item.title}
                                </span>
                              ))}
                              {media.length > 6 && (
                                <span>+{media.length - 6}</span>
                              )}
                            </div>
                          </div>
                    ...
                    {!lessons.length && (
                      <div className="empty-state library-empty-grade">
                        <BookOpen size={32} />
                        <p>
                          {search
                            ? "لا يوجد درس مطابق للبحث في هذا الصف."
                            : "لا توجد دروس في هذا الصف بعد."}
                        </p>
                        {canManage && (
                          <button
                            className="secondary-btn"
      ...
      {viewerLessonId && (
        <StudentLessonViewer
          data={data}
          lesson={(data.contentLibrary || []).find(
            (item) => String(item.id) === String(viewerLessonId),
          )}
          index={viewerIndex}
      ...
      {editorOpen && (
        ...
        >
          <aside
            className="library-lesson-editor"
            role="dialog"
            aria-modal="true"
            aria-label="محرر الدرس"
            onClick={(event) => event.stopPropagation()}
          >
            <header>
              <div>
                <span className="eyebrow">
                  {form.id ? "تعديل الدرس" : "درس جديد"}
                </span>
                <h2>{form.title || "إنشاء محتوى الدرس"}</h2>
                <p>
                  الكتاب الرئيسي للصف يُستخدم تلقائيًا؛ حدد الصفحات وأرفق
                  الوسائط فقط.
                </p>
              </div>
              <button
            ...
            <div className="library-editor-textbook-state">
              <BookOpen size={22} />
              <div>
                <strong>
                  {getGradeTextbook(data, form.grade)?.fileName ||
                    `كتاب ${form.grade} غير مرفوع`}
                </strong>
                <small>
                  {getGradeTextbook(data, form.grade)
                    ? "سيظهر تلقائيًا في الحصة ضمن نطاق الصفحات المحدد."
                    : "يمكن حفظ الدرس الآن، لكن يجب رفع الكتاب من بطاقته الدائمة."}
                </small>
              </div>
            </div>
            <div className="library-editor-grid">
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setForm({ ...form, lessonDate: event.target.value })
                  }
                />
              </label>
              <label>
              ...
              <label>
                ...
                <input
                  ...
                  onChange={(event) =>
                    setForm({ ...form, pageStart: event.target.value })
                  }
                />
              </label>
              <label>
                ...
                <input
                  type="number"
                  min={form.pageStart || 1}
                  value={form.pageEnd}
                  onChange={(event) =>
                    setForm({ ...form, pageEnd: event.target.value })
                  }
                />
              </label>
              <label className="span-2">
                ...
                <textarea
                  ...
                  placeholder="أهداف الدرس والنقاط المهمة وخطة الشرح..."
                />
              </label>
              <label className="span-2">
                الواجب المنزلي
                <textarea
                  ...
                  placeholder="اكتب واجب الدرس؛ سيظهر في تسجيل الحصة والتقارير."
                />
              </label>
              <label>
                صفحات أسئلة نهاية الدرس من
                <input
                  type="number"
                  min={form.pageStart || 1}
                  value={form.questionPageStart ?? ""}
                  onChange={(event) =>
                    setForm({ ...form, questionPageStart: event.target.value })
                  }
                />
              </label>
              <label>
                ...
                <input
                  type="number"
                  min={form.questionPageStart || form.pageStart || 1}
                  value={form.questionPageEnd ?? ""}
                  onChange={(event) =>
                    setForm({ ...form, questionPageEnd: event.target.value })
                  }
                />
              </label>
              <div className="span-2 library-ocr-panel">
                <div>
                  <ScanText size={24} />
                  <span>
                    <strong>استخراج الأسئلة بـ OCR العربي</strong>
                    <small>
                      يكتشف صفحات الأسئلة تلقائيًا قرب نهاية الدرس، أو يقرأ
                      نطاقًا تحدده يدويًا، ثم يضع الأسئلة هنا للمراجعة قبل
                      إدخالها بنك الألعاب.
                    </small>
                  </span>
                </div>
                <label className="library-ocr-source">
                  مصدر PDF
                  <select
                    value={form.ocrSourceKind || "textbook"}
                    onChange={(event) =>
                      setForm({ ...form, ocrSourceKind: event.target.value })
                    }
                  >
                ...
                <div className="library-ocr-actions">
                  <button
                    ...
                    onClick={() => void autoDetectLessonOcr()}
                  >
                    <Sparkles size={16} />
                    {ocrRunning
                      ? "جارٍ التحليل..."
                      : "اكتشاف صفحات الأسئلة تلقائيًا"}
                  </button>
                  <button
                    className="secondary-btn"
                    type="button"
                    disabled={ocrRunning || busy}
                    onClick={() => void runLessonOcr()}
                  >
                    <ScanText size={16} />
                    استخراج الأسئلة من PDF (صفحات محددة)
                  </button>
                  {ocrRunning && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setOcrStatus("جارٍ إلغاء عملية OCR بأمان...");
                      }}
                    >
                      <X size={16} />
                      إلغاء
              ...
              <OcrQuestionReview
                questions={form.ocrReviewQuestions || []}
                onChange={(ocrReviewQuestions) =>
                  setForm((current) => ({ ...current, ocrReviewQuestions }))
                }
              />
              <label className="span-2">
                أسئلة نهاية الدرس من الكتاب
                <textarea
                  rows="10"
                  ...
                  placeholder="استخدم زر OCR أو الصق الأسئلة وإجاباتها هنا. اكتب الإجابة بصيغة: الإجابة: ... الأسئلة التي لا تحتوي إجابة تبقى للمراجعة ولا تدخل الألعاب الآلية حتى تصحيحها."
                />
              </label>
              <label className="span-2">
                الوسوم
                <input
                  ...
                  placeholder="مصر، خريطة، حضارة"
                />
              </label>
            </div>

            <section className="library-editor-section">
              <div className="library-editor-section-head">
                <div>
                  <strong>خط سير الحصة</strong>
                  <small>يظهر بنفس الترتيب داخل وضع الحصة.</small>
                </div>
              </div>
              <div className="library-sequence-row">
                {Object.entries(flowLabels).map(([key, label]) => (
                  <button
                    key={key}
                    type="button"
                    className={form.sequence.includes(key) ? "active" : ""}
            ...
            <section className="library-editor-section">
              <div className="library-editor-section-head">
                <div>
                  <strong>وسائط الدرس الدائمة</strong>
                  <small>
                    صور، فيديو، صوت، PDF، PowerPoint ومستندات؛ تُربط تلقائيًا
                    بالدرس وتظهر في وضع الحصة.
                  </small>
                </div>
                <button
                  className="secondary-btn"
                ...
                >
                  <Plus size={15} /> إضافة ملفات
                </button>
              </div>
              <div className="library-editor-media-grid">
                {[...editingMedia, ...pendingMedia].map((item) => {
                  const pending = pendingMedia.some(
                    (entry) => String(entry.id) === String(item.id),
                  );
                  return (
                    <article key={item.id}>
                      <span className="library-editor-media-icon">
                        <MediaIcon type={item.type} />
                      </span>
                      <div>
                        <strong>{item.title}</strong>
                        <small>
                          {mediaLabels[item.type] || "ملف"}{" "}
                          {item.fileSize
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (pending) {
                            ...
                            deleteAsset(item.assetId).catch(() => {});
                          } else
                ...
                {!editingMedia.length && !pendingMedia.length && (
                  <div className="library-editor-empty-media">
                    <FolderOpen size={28} />
                    <span>لا توجد وسائط بعد.</span>
                  </div>
                )}
              </div>
            </section>
            ...
            <footer>
              ...
              >
                <Save size={16} />
                {busy ? "جارٍ الحفظ..." : "حفظ الدرس وكل محتواه"}
              </button>
            </footer>
          </aside>
        </div>
