...
import { CheckSquare, Eye, Printer, Search, ShieldCheck, Square } from 'lucide-react';
...
function CardField({ label, value, icon }) {
  return (
...
function FrontCard({ student }) {
  ...
  return (
    <article className="student-card-face front-card">
      <span className="card-frame-corner corner-tl" />
      <span className="card-frame-corner corner-tr" />
      <span className="card-frame-corner corner-bl" />
      <span className="card-frame-corner corner-br" />

      <div className="front-hero">
        <div className="front-hero-banner">
          <span>{identity.schoolName}</span>
          <strong>{identity.teacherTitle}</strong>
        </div>
        ...
        <div className="front-hero-footer">
          <div className="front-hero-mark">المبدع</div>
          <div className="front-hero-caption">لتعليم ممتع</div>
          <div className="front-hero-sub">خريطة العالم في العصور الوسطى</div>
        </div>
      </div>

      <div className="front-details">
        <div className="front-brand">
          <div className="front-brand-mark">
            <div className="brand-emblem">
              <span className="brand-emblem-feather">❦</span>
              <span className="brand-emblem-book">المبدع</span>
            </div>
          </div>
          <div className="front-brand-copy">
        ...
        <div className="front-info-list">
          <CardField label="اسم الطالب" value={student.name} icon="👤" />
          <CardField label="كود الطالب" value={student.code} icon="🪪" />
          <CardField label="الصف الدراسي" value={student.grade} icon="🎓" />
          <CardField label="المجموعة" value={student.group || '—'} icon="👥" />
          <CardField label="العام الدراسي" value={year} icon="📅" />
        </div>
  ...
  return (
    <article className="student-card-face back-card">
      ...
      <div className="back-left">
        <div className="back-section-title">تعليم ممتع</div>
        <div className="back-benefits">
          <div><span>◎</span><p>يستخدم للحضور والانصراف</p></div>
          <div><span>◎</span><p>متابعة الدرجات والتقارير</p></div>
          <div><span>◎</span><p>معًا نحو التميز والنجاح</p></div>
          <div><span>◎</span><p>التعليم الممتع هو طريق المستقبل</p></div>
        </div>
      </div>
      ...
      <div className="back-right">
        <div className="back-logo-emblem">
          <span className="brand-emblem-feather">❦</span>
      ...
      <div className="student-card-workbench no-print">
        ...
        <aside className="card-print-info panel">
          ...
          <div className="selection-actions">
            <button className="secondary-btn" onClick={selectAllVisible} disabled={!visibleIds.length || allVisibleSelected}><CheckSquare size={16} /> تحديد الكل</button>
            <button className="secondary-btn" onClick={clearSelection}><Square size={16} /> إلغاء التحديد</button>
          </div>
          ...
          <div className="card-print-actions">
            <button className="primary-btn icon-button" onClick={() => window.print()}><Printer size={18} /> طباعة {printMode === 'both' ? 'الوجهين' : (printMode === 'front' ? 'الوجه الأمامي' : 'الوجه الخلفي')}</button>
          </div>
        </aside>
      </div>
