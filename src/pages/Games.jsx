...
export default function Games({ data, updateData }) {
  ...
  const units = useMemo(() => [...new Set(filterQuestions({ gradeKey }).map((q) => q.unit))], [gradeKey]);
  ...
  useEffect(() => {
    ...
    if (seconds <= 0) { setLocked(true); setCombo(0); setFeedback(`انتهى الوقت — الإجابة الصحيحة: ${current.answer}`); playTone('wrong'); return; }
    const timer = setTimeout(() => setSeconds((s) => s - 1), 1000);
    return () => clearTimeout(timer);
  }, [seconds, mode, locked, current]);

  const poolFor = (selectedMode) => {
    if (selectedMode === 'truefalse') return filtered.filter((q) => q.type === 'tf');
    if (selectedMode === 'character') return filtered.filter((q) => q.type === 'character');
    if (selectedMode === 'timeline') return filtered.filter((q) => q.type === 'timeline');
    return filtered.filter((q) => q.type === 'mcq');
  };

  const startMode = (selectedMode) => {
    let pool = recentSafe(poolFor(selectedMode), history);
    if (!pool.length) { setFeedback('لا توجد أسئلة كافية لهذا الاختيار.'); return; }
    const count = ['timeline','character','matching'].includes(selectedMode) ? Math.min(4,pool.length) : Math.min(10,pool.length);
    const picked = shuffle(pool).slice(0,count);
    setMode(selectedMode); setRound(picked); setIndex(0); setScore(0); setCombo(0); setLocked(false);
    setFeedback(''); setSeconds(20); setBattleScores({one:0,two:0}); setActivePlayer('one');
    setTimelineOrder(picked[0]?.events ? shuffle(picked[0].events) : []); setCharacterGuess(''); setMatchSelection([]);
  ...
  const award = (correct, base = 10) => {
    if (mode === 'battle' || mode === 'teams') {
      ...
      setScore((v)=>v+bonus); setCombo((v)=>v+1);
    } else setCombo(0);
  };

  const answerChoice = (choiceIndex) => {
    if (locked || !current) return;
    setLocked(true); persist(current.id);
    const correct = choiceIndex === current.answerIndex;
    award(correct, mode === 'wheel' ? 15 : mode === 'surprise' ? 20 : 10);
    if (correct) { setFeedback(powerUp === 'x2' ? 'إجابة صحيحة — النقاط مضاعفة!' : 'إجابة صحيحة، ممتاز!'); playTone('correct'); encourageStudent('excellent', selectedStudent?.name || 'يا بطل', data.settings); }
    else { setFeedback(`الإجابة الصحيحة: ${current.answer} — راجع: ${current.topic}`); playTone('wrong'); }
  };

  const submitCharacter = () => {
    if (!current || locked) return; setLocked(true); persist(current.id);
    const correct = characterGuess.trim().replace(/\s+/g,' ') === current.answer.trim().replace(/\s+/g,' ');
    award(correct,15); setFeedback(correct?'ممتاز! اكتشفت الشخصية.':`الشخصية هي: ${current.answer}`); playTone(correct?'correct':'wrong');
  };

  const moveTimeline = (event, direction) => setTimelineOrder((prev)=>{ const copy=[...prev]; const i=copy.indexOf(event),t=i+direction; if(t<0||t>=copy.length)return prev; [copy[i],copy[t]]=[copy[t],copy[i]]; return copy; });
  const submitTimeline = () => { if(!current||locked)return; setLocked(true); persist(current.id); const correct=timelineOrder.every((e,i)=>e===current.events[i]); award(correct,20); setFeedback(correct?'ترتيب صحيح بالكامل +20 نقطة':`الترتيب الصحيح: ${current.events.join(' ← ')}`); playTone(correct?'correct':'wrong'); };

  const submitMatching = () => {
    if (!current || locked) return;
    const correct = matchSelection.length === 1 && matchSelection[0] === current.answerIndex;
    setLocked(true); persist(current.id); award(correct,15);
    setFeedback(correct?'مطابقة صحيحة!':`المطابقة الصحيحة: ${current.answer}`); playTone(correct?'correct':'wrong');
  };

  const finishRound = () => {
    const xp = mode === 'battle' || mode === 'teams' ? Math.max(battleScores.one,battleScores.two) : score;
    const result = { id:Date.now(), studentId:Number(playerOne)||null, secondStudentId:['battle','teams'].includes(mode)?Number(playerTwo)||null:null, mode, gradeKey, score:mode==='battle'||mode==='teams'?battleScores.one:score, secondScore:['battle','teams'].includes(mode)?battleScores.two:null, xp, date:new Date().toISOString().slice(0,10) };
    ...
    updateData({...data,gameResults:[...(data.gameResults||[]),result],achievements}); setMode('result'); playTone('win');
  };

  const nextQuestion = () => {
    if(index>=round.length-1){finishRound();return;}
    const next=index+1; setIndex(next); setLocked(false); setFeedback(''); setSeconds(20); setTimelineOrder(round[next]?.events?shuffle(round[next].events):[]); setCharacterGuess(''); setMatchSelection([]);
  ...
  return <section className="page"><div className="page-heading"><div><span className="eyebrow">محرك الألعاب الاحترافي</span><h2>الألعاب التعليمية</h2><p>جولات، فرق، نقاط، مستويات، مؤثرات، ومنع تكرار الأسئلة.</p></div></div>
    <div className="panel game-control-panel"><select value={gradeKey} onChange={(e)=>{setGradeKey(e.target.value);setUnit('all')}}>{gradeOptions.map((g)=><option key={g.key} value={g.key}>{g.label}</option>)}</select><select value={unit} onChange={(e)=>setUnit(e.target.value)}><option value="all">كل الوحدات</option>{units.map((u)=><option key={u} value={u}>{u}</option>)}</select><select value={playerOne} onChange={(e)=>setPlayerOne(e.target.value)}>{data.students.map((s)=><option key={s.id} value={s.id}>{s.name}</option>)}</select><select value={playerTwo} onChange={(e)=>setPlayerTwo(e.target.value)}>{data.students.map((s)=><option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
    {feedback&&<div className="game-feedback bad">{feedback}</div>}
    <div className="professional-games-grid">{modes.map(([id,icon,title,description])=><button className="pro-game-card" key={id} onClick={()=>startMode(id)}><span>{icon}</span><h3>{title}</h3><p>{description}</p></button>)}</div>
