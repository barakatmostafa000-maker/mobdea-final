...
const boardFontOptions = [
  {
    value:
      "'Noto Naskh Arabic', 'Droid Arabic Naskh', 'Traditional Arabic', serif",
    probe: "Noto Naskh Arabic",
    label: "نسخ — واضح وسلس",
  },
  {
    value: "'Aref Ruqaa', 'Urdu Typesetting', 'Segoe Print', cursive",
    probe: "Aref Ruqaa",
    label: "رقعة — كتابة سريعة",
  },
  {
    value: "'Amiri', 'Noto Naskh Arabic', 'Times New Roman', serif",
    probe: "Amiri",
    label: "أميري — تاريخي أنيق",
  },
  {
    value:
      "'Noto Kufi Arabic', 'Droid Arabic Kufi', 'Noto Sans Arabic', sans-serif",
    probe: "Noto Kufi Arabic",
    label: "كوفي — عناوين قوية",
  },
  {
    value: "'Reem Kufi', 'Noto Sans Arabic', Arial, sans-serif",
    probe: "Reem Kufi",
    label: "ريم كوفي — عرض مميز",
  },
];
  ...
  if (template === "history") {
    try {
      const reference = await dataUrlToImage(
        "/identity/class-board-history-v14.jpg",
      );
    ...
    >
      ...
      {template.fields.map((field) => {
        ...
        return (
          <div
            key={field.key}
            className={`board-card-field tone-${field.tone || "ink"} ${value ? "" : "placeholder"} ${displayValue ? "" : "empty-mask"}`}
            style={{
              left: `${field.x}%`,
              top: `${field.y}%`,
              width: `${field.w}%`,
              height: `${field.h}%`,
              fontSize: `clamp(8px, ${boardCardFieldScale(field, displayValue || field.label)}cqw, ${field.size || 24}px)`,
            }}
          >
            {displayValue
              ? displayValue
      ...
      {selected && (
        ...
        >
          <button type="button" onClick={updateFields}>
            تعديل النص
          </button>
          {template.imageZone && (
...
async function drawBoardCardToCanvas(ctx, action) {
  ...
  for (const field of template.fields) {
    ...
    const startY = fy - ((lines.length - 1) * lineHeight) / 2;
    lines.forEach((line, index) =>
      ctx.fillText(line, fx, startY + index * lineHeight, fw - 10),
    );
  }
  ctx.restore();
}
...
async function drawCountryCardToCanvas(ctx, action) {
  ...
  for (const field of countryCardFields(card)) {
    ...
    const size = fitCountryCanvasText(
      ctx,
      value,
      fw - 10,
      requested,
...
function CanvasOverlay({
  ...
  const onPointerMove = (event) => {
    ...
    for (const sample of samples) {
      ...
      if (
        !previous ||
        Math.hypot(point.x - previous.x, point.y - previous.y) >= 1.2
      ) {
        currentStroke.current.points.push({
          ...
          t: Math.max(
            1,
            Math.round(Number(sample.timeStamp || performance.now())),
          ),
        });
      }
    }
  ...
  return (
    <div
      ...
      onDrop={(event) => {
        ...
        const x =
          ((event.clientX - rect.left) / rect.width) * BOARD_CANVAS_WIDTH;
        const y =
          ((event.clientY - rect.top) / rect.height) * BOARD_CANVAS_HEIGHT;
      ...
      {!hasResourceHeader && template === "history" && (
        ...
        >
          <img
            ...
            src="/identity/class-board-history-v14.jpg"
            alt=""
          />
        </div>
      )}
...
export default function ClassMode({ data, updateData, navigate }) {
  ...
  const previousPointSession = useMemo(() => {
    const history = (
      Array.isArray(data.classPointSessions) ? data.classPointSessions : []
    )
      ...
      .filter(
        (item) =>
          !current?.group ||
          !item?.group ||
          String(item.group) === String(current.group),
      )
      .slice()
      .sort(
        (left, right) =>
          new Date(right?.updatedAt || right?.date || 0).getTime() -
          new Date(left?.updatedAt || left?.date || 0).getTime(),
      );
        ...
        students.map((student) => {
          ...
          const previousPoints =
            ...
            Object.prototype.hasOwnProperty.call(
              ...
              ? Math.max(
                  0,
                  Number(previousPointSession.points[student.id] || 0),
                )
              : currentPoints;
          return [
            student.id,
  ...
  const displayedStudents = useMemo(() => {
    ...
    return rankedStudents
      ...
      .sort(
        (left, right) =>
          Number(studentProgress[right.id]?.delta || 0) -
            Number(studentProgress[left.id]?.delta || 0) ||
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      try {
        ...
        /* Browser may not expose orientation unlock. */
      }
    };
  }, []);
  ...
  useEffect(() => {
    ...
    setClassPage(
      clampLessonPage(
        rememberedPage || selectedResource?.pageStart || 1,
        selectedResource,
        1,
      ),
    );
  ...
  const boardToolsAvailable =
    contentMode === "board" ||
    Boolean(displayResource && ["pdf", "images"].includes(contentMode));
  ...
  useEffect(() => {
    ...
    const upperBound = selectedResource?.pageEnd
      ? Math.min(Number(selectedResource.pageEnd), renderedPdf.pageCount)
  ...
  const adjustPoints = (student, delta) => {
    ...
    void updateData((latest) => {
      const nextStudents = (latest.students || []).map((item) =>
        String(item.id) === String(student.id)
          ? {
              ...item,
              points: Math.max(0, Number(item.points || 0) + change),
              updatedAt: new Date().toISOString(),
            }
          : item,
      );
      const scopedStudents = current?.group
        ? nextStudents.filter(
            (item) => String(item.group || "") === String(current.group),
          )
        : nextStudents;
      const sessionRecord = {
        id: pointSessionKey,
        ...
        points: Object.fromEntries(
          scopedStudents.map((item) => [
            item.id,
            Math.max(0, Number(item.points || 0)),
          ]),
        ),
      };
      const classPointSessions = [
        sessionRecord,
        ...(Array.isArray(latest.classPointSessions)
          ? latest.classPointSessions
          : []
        ).filter((item) => String(item?.id || "") !== pointSessionKey),
      ].slice(0, 240);
  ...
  const convertRecentHandwriting = async () => {
    ...
    try {
      ...
      const nextAction = {
        ...
        handwritingSourceStrokes: strokes.map((stroke) => ({
          ...stroke,
      ...
      const fontLabel =
        ...
        "الخط المختار";
      setShareNotice(
        `تم تنسيق خط اليد كنمط «${presetLabel}» بخط «${fontLabel}»${Number.isFinite(confidence) ? ` — دقة ${Math.round(confidence)}%` : ""}.`,
      );
    } catch (error) {
      setShareNotice(
    ...
    >
      <ClassModeViewport.Header>
        <div className="classmode-top-header">
          ...
          <div className="classmode-header-tabs">
            <button
              type="button"
              className={contentMode === "pdf" ? "active" : ""}
              onClick={() => switchContentMode("pdf")}
              title="كتاب الشرح PDF"
            >
              <FileText size={22} />
              <span>PDF</span>
            </button>
            <button
              type="button"
              className={contentMode === "board" ? "active" : ""}
            ...
            <button
              type="button"
              className={contentMode === "maps" ? "active" : ""}
              onClick={() => switchContentMode("maps")}
              title="الخرائط"
            >
              <MapIcon size={22} />
              <span>الخرائط</span>
            ...
            <button
              type="button"
              className={contentMode === "images" ? "active" : ""}
              onClick={() => switchContentMode("images")}
              title="الصور"
            >
              <FileImage size={22} />
              <span>الصور</span>
            ...
            <button
              type="button"
              className={contentMode === "videos" ? "active" : ""}
              onClick={() => switchContentMode("videos")}
              title="الفيديوهات"
            >
              <Video size={22} />
              <span>الفيديو</span>
            ...
            <button
              type="button"
              className={contentMode === "audio" ? "active" : ""}
              onClick={() => switchContentMode("audio")}
              title="الصوتيات"
            >
              <Volume2 size={22} />
              <span>الصوت</span>
            ...
            <button
              type="button"
              className={contentMode === "slides" ? "active" : ""}
              onClick={() => switchContentMode("slides")}
              title="PowerPoint"
            >
            ...
            <button
              type="button"
              className={contentMode === "games" ? "active" : ""}
              onClick={() => switchContentMode("games")}
              title="ألعاب الدرس داخل الحصة"
            >
              <Gamepad2 size={22} />
              <span>الألعاب</span>
            </button>
            ...
            <div className="classmode-board-frame">
              ...
              {boardToolsAvailable && (
                ...
                >
                  <LayoutGrid size={17} />
                  <span>
                    {boardControlsOpen ? "إخفاء الأدوات" : "أدوات السبورة"}
                  </span>
                </button>
              )}
              {contentMode === "board" && (
                  ...
                  {cardDrawerSection === "education" ? (
                    ...
                    <>
                      ...
                      <div className="classmode-card-drawer-list country-card-drawer-list">
                        ...
                        >
                          <img src={DEFAULT_COUNTRY_CARD.asset} alt="" />
                          <span>بطاقة افتراضية</span>
                        </button>
                        {countryCardsForCategory(countryCategory).map(
                          (card) => (
                            <button
                              key={card.key}
                              type="button"
                ...
                >
                  ...
                  {contentMode === "games" ? (
                    ...
                    <>
                        ...
                        <div className="classmode-resource-preview">
                          <div className="resource-preview-head">
                            ...
                            {modeResources.length > 1 && (
                              <div
                                className="classmode-inline-media-switcher"
                                role="group"
                                aria-label="التبديل بين ملفات النوع الحالي"
                              >
                                <button
                                  type="button"
                                  onClick={() => cycleModeResource(-1)}
                                ...
                                <select
                                  ...
                                  aria-label="اختيار ملف الدرس"
                                >
                                  {modeResources.map((resource, index) => (
                                    <option
                                      key={resource.id}
                                      value={resource.id}
                                    >
                      ...
                      {(contentMode === "board" ||
                        (displayResource &&
                          ["pdf", "images"].includes(contentMode))) && (
                        <CanvasOverlay
            ...
            {boardToolsVisible && (
              ...
              >
                <div className="classmode-tool-group compact classmode-color-row">
                  {["#111827", "#2563eb", "#dc2626", "#d7ad35", "#7c3aed"].map(
                    (c) => (
                      <button
                        key={c}
                        type="button"
                ...
                <div className="classmode-tool-group compact">
                  ...
                  <button onClick={clearBoard} type="button">
                    <RotateCcw size={17} /> مسح الكل
                  </button>
                  <button onClick={persistCurrentBoardLayer} type="button">
                    <Save size={17} /> حفظ طبقة الكتابة
                  </button>
                </div>
              </div>
            )}
          ...
          <aside className="classmode-side-column">
            <article className="panel classmode-side-panel classmode-management-actions-panel">
              ...
              <div className="classmode-management-actions-grid">
                ...
                <button
                  type="button"
                  className="secondary-btn"
                  onClick={persistCurrentBoardLayer}
                >
                  <Save size={16} />
            ...
            <article className="panel classmode-side-panel classmode-students-panel">
              ...
              <div className="classmode-students-list">
                {displayedStudents.map((student) => {
                  ...
                  return (
                    ...
                    >
                      <button
                        className="classmode-student-row-main"
                        onClick={() => {
                          setSelectedStudent(student);
                          setPhraseMenu("");
                        }}
                      ...
                      >
                        ...
                        <b className={`attendance-dot ${status || "pending"}`}>
                          {statusLabels[status] || "لم يسجل"}
                        </b>
                        <span className="student-score">
                          {score}
                          {Number(studentProgress[student.id]?.delta || 0) >
                            0 && (
                            <small>
                              ↑ +{studentProgress[student.id].delta}
                            </small>
                          )}
                        </span>
                      </button>
              ...
              <div className="classmode-student-commandbar">
                <div
                  className={`classmode-command-selected ${selectedStudent ? "has-student" : ""}`}
                >
                  <span>الطالب المحدد</span>
                  <strong>
                    {selectedStudent?.name || "اضغط على اسم طالب أولًا"}
                  </strong>
                </div>
                <div className="classmode-command-buttons">
                  <button
                    type="button"
                    className={
                      phraseMenu === "positive" ? "active positive" : "positive"
                    }
                    disabled={!selectedStudent}
                    onClick={() =>
                      setPhraseMenu((current) =>
                  ...
                  <button
                    ...
                    disabled={!selectedStudent}
                    onClick={() =>
                      selectedStudent && adjustPoints(selectedStudent, 1)
                    }
                      ...
                      {(phraseMenu === "positive"
                        ...
                        <button
                          ...
                          onClick={() =>
                            sayPhrase(
                              phrase,
                              phraseMenu === "positive" ? "excited" : "calm",
                            )
                          }
                        >
                          <Volume2 size={14} />
                    ...
                    <div className="classmode-command-add-phrase">
                      <input
                        value={
                          phraseMenu === "positive"
                            ? newPhraseText
                            : newCorrectivePhraseText
                        }
                        onChange={(event) =>
                          phraseMenu === "positive"
                            ? setNewPhraseText(event.target.value)
                            : setNewCorrectivePhraseText(event.target.value)
                        }
                        onKeyDown={(event) => {
      ...
      <ClassModeViewport.Overlays>
        ...
        {view === "students" && students.length > 0 && (
          ...
          >
            <aside
              className="classmode-student-drawer"
              role="dialog"
              aria-modal="true"
              aria-label="طلاب الحصة"
              onClick={(event) => event.stopPropagation()}
            >
              <header>
                <div>
                  <span className="eyebrow">
                    طلاب الحصة ({students.length})
                  </span>
                  <h3>الحضور والنقاط الفورية</h3>
                </div>
                <button
                  className="icon-action"
                  type="button"
              ...
              <div className="classmode-student-drawer-list">
                {displayedStudents.map((student) => {
                  ...
                  return (
                    <div
                      className="classmode-student-drawer-row"
                      key={student.id}
                    >
                      <div>
                        <strong>{student.name}</strong>
                      ...
                      <div className="drawer-attendance-actions">
                        ...
                        <button
                          className={
                            status === "late" ? "selected late" : "late"
                          }
                        ...
                        <button
                          className={
                            status === "absent" ? "selected absent" : "absent"
                          }
