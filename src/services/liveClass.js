...
function publicConfig(configLike = {}) {
  ...
  const workspaceId = safeTrim(configLike.workspaceId || "", 80).replace(
    /[^a-zA-Z0-9_-]/g,
    "",
  );
...
export function buildLiveStudentLink(
  payload,
  ...
  const workspaceId = safeTrim(payload?.workspaceId || "", 80).replace(
    /[^a-zA-Z0-9_-]/g,
    "",
  );
  ...
  base.searchParams.set(
    "shareData",
    encodeSharePayload({ ...payload, roomId, joinCode, endpoint, workspaceId }),
  );
  const link = base.toString();
  if (!validateLiveStudentLink(link))
    throw new Error("تعذر تجهيز رابط الطالب بصورة صحيحة.");
  return link;
}
...
export async function createLiveRoom(settings, metadata = {}) {
  ...
  const response = await timeoutFetch(
    buildCloudUrl(config.endpoint, "/live/rooms"),
    {
      method: "POST",
  ...
  if (!response.ok) {
    throw new Error(
      await readErrorBody(
        response,
        `تعذر إنشاء الحصة المباشرة (${response.status}).`,
      ),
    );
  }
...
export async function joinLiveRoom(configLike, roomId, joinCode, profile = {}) {
  ...
  if (!response.ok) {
    throw new Error(
      await readErrorBody(response, `تعذر دخول الحصة (${response.status}).`),
    );
  }
...
export async function postLiveEvent(configLike, roomId, token, event) {
  ...
  if (bodySize(payload) > MAX_EVENT_BYTES)
    throw new Error("بيانات الحدث أكبر من المسموح.");
  const response = await timeoutFetch(
    buildCloudUrl(
      config.endpoint,
      `/live/rooms/${encodeURIComponent(roomId)}/events`,
    ),
    {
      method: "POST",
  ...
  if (!response.ok) {
    throw new Error(
      await readErrorBody(
        response,
        `تعذر إرسال حدث الحصة (${response.status}).`,
      ),
    );
  }
...
export async function fetchLiveEvents(configLike, roomId, token, after = 0) {
  ...
  if (!response.ok) {
    throw new Error(
      await readErrorBody(
        response,
        `تعذر استقبال أحداث الحصة (${response.status}).`,
      ),
    );
  }
  return response.json();
...
export async function listLiveParticipants(configLike, roomId, teacherToken) {
  ...
  const response = await timeoutFetch(
    buildCloudUrl(
      config.endpoint,
      `/live/rooms/${encodeURIComponent(roomId)}/participants`,
    ),
    {
      method: "GET",
  ...
  if (!response.ok) {
    throw new Error(
      await readErrorBody(
        response,
        `تعذر قراءة قائمة الطلاب (${response.status}).`,
      ),
    );
  }
  return response.json();
...
export async function updateLiveParticipant(
  ...
  if (!response.ok) {
    throw new Error(
      await readErrorBody(response, `تعذر تحديث الطالب (${response.status}).`),
    );
  }
  return response.json();
}
...
export async function closeLiveRoom(configLike, roomId, teacherToken) {
  ...
  if (!response.ok) {
    throw new Error(
      await readErrorBody(response, `تعذر إنهاء الحصة (${response.status}).`),
    );
  }
  return response.json();
}
