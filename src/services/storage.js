...
const normalizeSettings = (settings = {}) => {
  ...
  return {
    ...
    teacherPin: safeTrim(settings.teacherPin ?? seedSettings.teacherPin, 6).replace(/\D/g, '').slice(0, 6),
    teacherPinHash: safeTrim(settings.teacherPinHash ?? seedSettings.teacherPinHash ?? '', 160),
    teacherPinSalt: safeTrim(settings.teacherPinSalt ?? seedSettings.teacherPinSalt ?? '', 80),
    staffRecoveryQuestion: safeTrim(settings.staffRecoveryQuestion ?? seedSettings.staffRecoveryQuestion ?? '', 160),
    staffRecoveryAnswerHash: safeTrim(settings.staffRecoveryAnswerHash ?? seedSettings.staffRecoveryAnswerHash ?? '', 160),
    staffRecoveryAnswerSalt: safeTrim(settings.staffRecoveryAnswerSalt ?? seedSettings.staffRecoveryAnswerSalt ?? '', 80),
