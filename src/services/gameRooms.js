...
function validateCreatedGameRoom(room = {}) {
  ...
  if (!roomId || !joinCode || teacherToken.length < 16) {
    throw new Error('استجابة خادم اللعب غير مكتملة. لم يتم إنشاء غرفة صالحة.');
  }
...
export async function createGameRoom(
  ...
  const created = validateCreatedGameRoom(await response.json());
  return {
    ...created,
...
export async function joinGameRoom(
  ...
  const roomId = safeTrim(joined.roomId || '', 120);
  ...
  if (!roomId || !participantId || participantToken.length < 12) {
    throw new Error('استجابة دخول الطالب للعبة غير مكتملة. أعد فتح الرابط.');
  }
  return {
    ...joined,
    roomId,
