...
import {
  releaseNativeAsset,
  stageBlobForNative,
} from "../../services/nativeAssetBridge";
import {
  parsePptxSlideLayout,
  readPptxRelationships,
  readPptxSlideSize,
} from "../../services/pptxLayout";
...
async function inflateRaw(bytes) {
  if (typeof DecompressionStream === "undefined") {
    throw new Error(
      "عارض PowerPoint الداخلي غير مدعوم في هذا المتصفح. افتح الملف على الجهاز.",
    );
  }
...
async function readZipEntries(arrayBuffer) {
  ...
  if (endOffset < 0)
    throw new Error("ملف PowerPoint غير صالح أو ليس بصيغة PPTX.");
  ...
  const read = async (name) => {
    ...
    if (!bytes)
      throw new Error(
        "يستخدم العرض ضغطًا غير مدعوم داخل التطبيق. افتحه على الجهاز.",
      );
    cache.set(key, bytes);
    return bytes;
  };
...
async function loadRelationshipImages(archive, relationships, objectUrls) {
  ...
  for (const imagePath of [
    ...
    const objectUrl = URL.createObjectURL(
      new Blob([imageBytes], { type: mediaMime(imagePath) }),
    );
...
async function parsePptxInBrowser(blob, url) {
  ...
  if (blob instanceof Blob) buffer = await blob.arrayBuffer();
    ...
    if (!response.ok)
      throw new Error("تعذر قراءة ملف PowerPoint من ذاكرة الجهاز.");
  ...
  if (!slideNames.length)
    throw new Error("لم يتم العثور على شرائح قابلة للعرض داخل الملف.");
  ...
  const presentationXml = presentationBytes
    ? parser.parseFromString(
        new TextDecoder().decode(presentationBytes),
        "application/xml",
      )
    : null;
  ...
  for (const slideName of slideNames) {
    ...
    const slideXml = parser.parseFromString(
      new TextDecoder().decode(slideBytes),
      "application/xml",
    );
    const texts = xmlNodes(slideXml, "t")
      .map((node) => node.textContent?.trim())
      .filter(Boolean);
    const slideNumber = Number(
      slideName.match(/slide(\d+)/i)?.[1] || slides.length + 1,
    );
    ...
    if (relationshipBytes) {
      const relationshipXml = parser.parseFromString(
        new TextDecoder().decode(relationshipBytes),
        "application/xml",
      );
      relationships = readPptxRelationships(relationshipXml, (target) =>
        resolveRelationshipTarget(slideName, target),
      );
    }

    const imageSources = await loadRelationshipImages(
      archive,
      relationships,
      objectUrls,
    );
    ...
    const layoutRelationship = [...relationships.values()].find((item) =>
      item.type.includes("/slideLayout"),
    );
    let layoutXml = null;
    ...
    if (layoutRelationship) {
      ...
      if (layoutBytes) {
        layoutXml = parser.parseFromString(
          new TextDecoder().decode(layoutBytes),
          "application/xml",
        );
        const layoutRelationshipBytes = await archive.read(
          relationshipsPartName(layoutRelationship.path),
        );
        if (layoutRelationshipBytes) {
          const relationshipXml = parser.parseFromString(
            new TextDecoder().decode(layoutRelationshipBytes),
            "application/xml",
          );
          layoutRelationships = readPptxRelationships(
            relationshipXml,
            (target) =>
              resolveRelationshipTarget(layoutRelationship.path, target),
          );
          layoutImageSources = await loadRelationshipImages(
            archive,
            layoutRelationships,
            objectUrls,
          );
  ...
  return {
    slides,
    cleanup: () =>
      objectUrls.forEach((urlItem) => URL.revokeObjectURL(urlItem)),
  };
}

async function parsePptxNative(blob) {
  if (!(blob instanceof Blob))
    throw new Error("ملف PowerPoint غير متاح داخل ذاكرة المنصة.");
  if (blob.size > 100 * 1024 * 1024)
    throw new Error("حجم ملف PowerPoint أكبر من الحد المدعوم للعرض الداخلي.");
...
export default function PptxPreview({ url, blob, title, onOpenExternal }) {
  ...
  useEffect(() => {
    ...
    const parser =
      globalThis.Capacitor?.isNativePlatform?.() && blob
    ...
    parser
      .then((result) => {
        ...
        if (!result.slides.length)
          throw new Error("لم يتم العثور على شرائح قابلة للعرض.");
  ...
  const counter = useMemo(
    () => (slides.length ? `${index + 1} من ${slides.length}` : ""),
    [index, slides.length],
  );

  if (status === "loading")
    return (
      <div className="pptx-preview-state">
        <Presentation size={44} />
        <strong>جارٍ تجهيز شرائح PowerPoint…</strong>
        <small>يتم فتح العرض داخل المنصة دون الخروج من وضع الحصة.</small>
      </div>
    );
  if (status !== "ready" || !slide)
    return (
      <div className="pptx-preview-state error">
        <Presentation size={44} />
        <strong>{error || "لا يوجد عرض متاح."}</strong>
        <button className="primary-btn" type="button" onClick={onOpenExternal}>
          <ExternalLink size={16} /> فتح الملف على الجهاز
        </button>
      </div>
    );
  ...
  return (
    <div className="pptx-preview" aria-label={`عرض ${title || "PowerPoint"}`}>
      <div
        className={`pptx-slide ${slide.elements?.length ? "layout-aware" : "fallback-layout"}`}
        ...
        {slide.elements?.length ? (
          ...
          <>
            {slide.images?.length > 0 && (
              <div
                className={`pptx-slide-images images-${Math.min(slide.images.length, 4)}`}
              >
                {slide.images.map((src, imageIndex) => (
                  <img key={`${slide.id}:${imageIndex}`} src={src} alt="" />
                ))}
              </div>
            )}
            <div className="pptx-slide-text">
              {slide.texts?.length ? (
                slide.texts.map((text, textIndex) =>
                  textIndex === 0 ? (
                    <h2 key={`${slide.id}:t:${textIndex}`}>{text}</h2>
                  ) : (
                    <p key={`${slide.id}:t:${textIndex}`}>{text}</p>
                  ),
                )
              ) : (
                <p>شريحة تحتوي عناصر رسومية بدون نص قابل للاستخراج.</p>
              )}
            </div>
          </>
        )}
