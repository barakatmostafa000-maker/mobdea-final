    ...
    <section className="span-2 ocr-review-panel" aria-label="مراجعة أسئلة OCR قبل الاعتماد">
      ...
      <div className="ocr-review-list">
        {questions.map((item, index) => (
          <article key={item.id} className={`${item.approved ? 'approved' : ''} ${String(item.answer || '').trim() ? 'answer-ready' : 'needs-answer'}`}>
            ...
            <label>الإجابة {!String(item.answer || '').trim() && <em className="ocr-answer-required">— مطلوبة قبل الاعتماد</em>}<input value={item.answer || ''} onChange={(event) => update(item.id, { answer: event.target.value, approved: false })} placeholder="اكتب الإجابة الصحيحة — لن يتم التخمين تلقائيًا"/></label>
            <footer>
