...
export function usePdfPage(source, page = 1, maxWidth = 1600) {
  ...
  useEffect(() => {
    ...
    const task = isNative
      ? blob
        ? renderNativePdfBlob(blob, page, maxWidth, cacheKey)
        : renderNativePdfPage(url, page, maxWidth)
      : blob
        ? renderWebPdfBlob(blob, page, maxWidth, cacheKey)
        : renderWebPdfPage(url, page, maxWidth, cacheKey || url);
