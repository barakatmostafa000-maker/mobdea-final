...
export function GeographyGlyph({ type = '', symbol = '●' }) {
  ...
  if (type === 'mountains') {
    return <svg className="geography-glyph-svg relief-glyph terrain-3d mountain-3d" viewBox="0 0 82 58" aria-hidden="true">{defs}
      <ellipse cx="42" cy="51" rx="35" ry="5" fill="#020202" opacity=".34"/>
      <g filter={`url(#${relief})`}>
        <path d="M4 49 24 17l12 19L49 6l29 43Z" fill={`url(#${earth})`}/>
        <path d="M49 6 36 36l13-9 29 22H54Z" fill="#4d3020" opacity=".82"/>
        <path d="M24 17 7 49h20l9-13Z" fill="#5c3924" opacity=".72"/>
        <path d="m17 29 7-12 7 11-7-4Zm23-9 9-14 11 17-11-7Z" fill="#fff7e7"/>
        <path d="M10 45c17-4 34-3 56 0" fill="none" stroke="#d9b878" strokeWidth="2" opacity=".6"/>
        <path d="M13 40c12-3 28-2 43 1" fill="none" stroke="#f1d79b" strokeWidth="1.5" opacity=".55"/>
      </g>
    </svg>;
  }
  if (type === 'plateaus') {
    return <svg className="geography-glyph-svg relief-glyph terrain-3d plateau-3d" viewBox="0 0 82 58" aria-hidden="true">{defs}
      <ellipse cx="42" cy="51" rx="34" ry="5" fill="#000" opacity=".3"/>
      <g filter={`url(#${relief})`}>
        <path d="M10 44 20 17 60 17 73 44 58 52 22 52Z" fill={`url(#${earthDark})`}/>
        <path d="M20 17 60 17 70 29 15 29Z" fill="#e1ba78"/>
        <path d="M15 29 70 29 73 44 58 52 22 52 10 44Z" fill={`url(#${earth})`}/>
        <path d="M20 17 60 17 54 22 25 22Z" fill="#ffe1a5" opacity=".72"/>
        <path d="M18 35c13-4 31-4 47 0M16 41c16-4 35-3 52 1" fill="none" stroke="#f2d39a" strokeWidth="1.5" opacity=".62"/>
      </g>
    </svg>;
  }
  if (type === 'plains') {
    ...
    <div className={`map-pro-stage map-style-${mapStyle} ${silent ? 'silent-map' : ''}`} onClick={onStageClick} onDragOver={(event) => event.preventDefault()} onDrop={handleDrop}>
      <div ref={transformRef} className="map-pro-transform" style={{ transform: `scale(${zoom})` }}>
        <svg viewBox="0 0 1000 620" preserveAspectRatio="xMidYMid meet" className="map-pro-svg" role="img" aria-label={ariaLabel}>
          <MapDefs />
          <rect className="map-pro-ocean" width="1000" height="620" fill="url(#mapOcean)" />
              ...
              return (
                <g key={id}>
                  <path
                    ...
                    fill={silent ? '#d9d6b8' : active ? '#d49a38' : countryFill(featureIndex)}
                    style={{ '--country-fill': silent ? '#d9d6b8' : active ? '#d49a38' : countryFill(featureIndex) }}
                    onClick={(event) => {
                      event.stopPropagation();
