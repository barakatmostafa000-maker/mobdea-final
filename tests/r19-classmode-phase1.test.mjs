...
test("true fullscreen uses presentation-only chrome and an exit control", () => {
  ...
  assert.match(
    css,
    /\.classmode-viewport\.presentation-fullscreen > \.classmode-viewport-header/,
  );
  assert.match(
    css,
    /\.classmode-viewport\.presentation-fullscreen \.classmode-viewport-students/,
  );
});
...
test("placing cards and teaching symbols returns to writing instead of trapping the tool", () => {
  assert.match(
    classMode,
    /\['historical-symbol', 'geographical-symbol', 'shape', 'arrow'\]\.includes\(action\.kind\)/,
  );
