...
test("OCR page number inputs can be cleared and retyped", () => {
  ...
  assert.doesNotMatch(
    contentLibrary,
    /value=\{form\.questionPageStart \|\| form\.pageStart \|\| 1\}/,
  );
});
...
test("Class Mode exposes one collapsible management surface", () => {
  ...
  assert.match(
    css,
    /\.classmode-viewport\.management-closed \.classmode-viewport-students/,
  );
  assert.match(
    css,
    /\.classmode-viewport\.management-open \.classmode-viewport-students/,
  );
});
...
test("Arabic and English OCR models are bundled into the Android app", () => {
  for (const name of ["ara.traineddata.gz", "eng.traineddata.gz"]) {
    ...
    assert.ok(
      fs.statSync(path).size > 100_000,
      `${name} is unexpectedly small`,
    );
  }
});
...
test("board pointer mapping is clamped to the exact visible canvas rectangle", () => {
  assert.match(
    classMode,
    /const localX = Math\.max\(0, Math\.min\(rect\.width/,
  );
  assert.match(
    classMode,
    /const localY = Math\.max\(0, Math\.min\(rect\.height/,
  );
...
test("professional map title is compact and centered while lesson rivers are thin", () => {
  ...
  assert.match(
    professionalMap,
    /map-pro-region-plaque" transform="translate\(385 18\)"/,
  );
...
test("collapsing management does not unmount live controls or student state", () => {
  ...
  assert.doesNotMatch(
    classMode,
    /\{managementOpen\s*&&\s*\(?\s*<ClassModeViewport\.Students>/,
  );
...
test("entering or leaving fullscreen does not reset the active board or resource state", () => {
  const match = classMode.match(
    /const toggleFullscreen = async \(\) => \{([\s\S]*?)\n  \};/,
  );
  assert.ok(match, "toggleFullscreen implementation is missing");
  assert.doesNotMatch(
    match[1],
    /setBoardActions|setSelectedResourceId|setContentMode|clearBoard/,
  );
...
test("Android keeps the functional plugins needed by the repaired classroom", () => {
  ...
  for (const plugin of [
    ...
    assert.ok(
      mainActivity.includes(`registerPlugin(${plugin})`),
...
test("500 MB textbook support remains aligned across storage staging PDF and OCR", () => {
  ...
  const nativeAsset = read(
    "android/app/src/main/java/com/mobdea/education/assets/MobdeaNativeAssetPlugin.java",
  );
  const nativePdf = read(
    "android/app/src/main/java/com/mobdea/education/pdf/MobdeaPdfRendererPlugin.java",
  );
  const nativeOcr = read(
