...
test('lesson map exposes the six core maps in one map drawer', () => {
  const source = read('src/components/maps/LessonMapStudio.jsx');
  assert.match(source, /CORE_REGION_KEYS = \['egypt', 'arab', 'africa', 'asia', 'europe', 'world'\]/);
...
test('map challenge exposes Egypt, Arab world, Africa, Asia, Europe and World', () => {
  const source = read('src/pages/MapChallenge.jsx');
  assert.match(source, /coreRegionKeys = \['egypt', 'arab', 'africa', 'asia', 'europe', 'world'\]/);
